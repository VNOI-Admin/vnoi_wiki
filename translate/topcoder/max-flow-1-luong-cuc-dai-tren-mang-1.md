...
**Yêu cầu**: với mỗi kênh truyền tải $e=[u,v]\in E$ cần xác định giá trị $f[u,v] (f[u,v]\le c[u,v])$ được gọi là luồng (flow) trên kênh $e$, sao cho $\sum_{v \in V} f[v,u]=\sum_{w\in V} f[u,w]$ ($\forall u\in V \setminus \{s,t\}$) (tổng luồng đi vào bằng tổng luồng đi ra). Hơn thế nữa $f(s,V)=\sum_{v\in V} f[s,v]$ là lớn nhất.
...
![](/uploads/translate/topcoder/max-flow-1-luong-cuc-dai-tren-mang-1/max_flow_img_1.jpg)
...
![](/uploads/translate/topcoder/max-flow-1-luong-cuc-dai-tren-mang-1/max_flow_2a.jpg)
![](/uploads/translate/topcoder/max-flow-1-luong-cuc-dai-tren-mang-1/max_flow_2b.jpg)
...
![](/uploads/translate/topcoder/max-flow-1-luong-cuc-dai-tren-mang-1/max_flow_3a.jpg)
...
![](/uploads/translate/topcoder/max-flow-1-luong-cuc-dai-tren-mang-1/max_flow_3b.jpg)
...
![](/uploads/translate/topcoder/max-flow-1-luong-cuc-dai-tren-mang-1/max_flow_1b.jpg)

## 3.4 Thuật toán Ford–Fulkerson

Từ ví dụ trên ta có thể đi đến thuật toán như sau:

$\text{bước }(1)$: Tạo mạng thặng dư $G'$ tương ứng cho mạng $G$ ban đầu

$\text{bước }(2)$: tìm một đường tăng luồng trên mạng thặng dư $G'$

  - nếu không tồn tại đường tăng luồng $\rightarrow$ kết thúc thuật toán

  - nếu tồn một đường tăng luồng $\rightarrow$ thực hiện tăng luồng trên mạng thặng dư và quay trở lại $\text{bước }(2)$
Khi thuật toán kết thúc $f(s,V')$ chính là giá trị luồng cực đại cần tìm.

Đến đây bạn đã có thể dùng thuật toán tìm kiếm trên đồ thị DFS (deep first search) hoặc BFS(breath first search) để tìm đường tăng luồng và cập nhật mạng thặng dư thuật toán này có độ phức tạp bằng *số lần tăng luồng* ($f^{*}$) nhân với *độ phức tạp của thật toán tìm kiếm đồ thị-$\mathcal{O}(E)$* và bằng $\mathcal{O}(|f^{*}| \times E)$. Sau đâu là code của thuật toán trên:
...
Như đã nói $\mathcal{O}(|f^{*}| \times E)$ là độ phức tạp của thuật toán Ford-Fulkerson nó phụ thuộc 2 yếu tố là tìm đường tăng luồng $\mathcal{O}(E)$ và số lần tăng luồng $f^{*}$ do đó ta có thể tối ưu 1 trong 2 hoặc cả 2 nếu muốn thuật toán chạy nhanh hơn. Trong mục này ta sẽ tìm hiểu cách để có thể giảm được số lần tăng luồng $f^{*}$ điều này phụ thuộc nhiều vào việc chọn đường tăng luồng nào để tăng, các phương pháp dưới đây đều có độ phức tạp là $\mathcal{O}(|f^{*}| \times E)$ nhưng đa số các trường hợp sẽ có độ tốt tăng dần theo thứ tự trình bày sau:
...
Cho một mạng gồm $n$ đỉnh với $p$ điểm phát $A_{1}, A_{2}, \ldots, A_{p}$ và q điểm thu $B_{1}, B_{2}, \ldots, B_{p}$. Mỗi cung của mạng được gán khả năng thông qua là số
...
Một lớp học khiêu vũ có N bạn nam $B_{1}, B_{2}, \ldots, B_{N}$ và M bạn nữ $G_{1}, G_{2}, \ldots, G_{M}$ ở buổi học đầu tiên các bạn nam được yêu cầu mời một bạn nữ làm bạn nhảy cùng trong cả khóa học theo khảo sát chúng ta biết được bảng giá trị like[i][j], like[i][j]=True nếu bạn nữ Gj chấp nhận lời đề nghị từ bạn nam Bi và like[i][j]=False ngược lại nếu bạn gái Gj không chấp nhận lời mời từ bạn nam Bi. Bạn hãy xác định số cặp nhảy nhiều nhất có thể của lớp học.

## Tập đại diện

 Một lớp học có n bạn nam, n bạn nữ. Cho m món quà lưu niệm, $(n \le m)$. Mỗi bạn có sở thích về một số món quà nào đó. Hãy tìm cách phân cho mỗi bạn nam tặng một món quà cho một bạn nữ thoả mãn:
