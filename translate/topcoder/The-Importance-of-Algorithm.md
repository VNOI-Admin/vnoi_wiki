...
Một cách tiếp cận vấn đề khác đó là thử "randomize" thuật toán theo một cách nào đó. Tuy không cải thiện tốc độ trong trường hợp xấu nhất, biện pháp này sẽ có hiệu quả với những trường hợp bình thường. **[QuickSort](https://en.wikipedia.org/wiki/Quicksort)** là một ví dụ điển hình cho việc randomize như vậy. Khi "suy biến", QuickSort sẽ sắp xếp một dãy trong $O(N^2)$, với $N$ là số phần tử của dãy. Nhưng nếu áp dụng random vào thuật toán, xác suất xảy ra suy biến là cực kì thấp, và độ phức tạp trung bình của QuickSort trở thành $O(N*Log(N))$. Có những thuật có độ phức tạp $O(N*Log(N))$ kể cả ở trường hợp xấu, nhưng chúng sẽ chậm hơn ở trường hợp bình thường. Dù cùng là $O(N*Log(N))$, QuickSort có hằng số $C$ nhỏ hơn, tức số lệnh cần thực hiện nhỏ hơn, ví dụ Quick Sort cần $C*N*Log(N)$ phép tính còn các thuật khác cần $2*C*N*Log(N)$ phép tính.

Một thuật toán khác dùng randomize để tìm trung vị của một dãy với độ phức tạp trung bình $O(N)$. Đây là cải tiến rất lớn so với việc sắp xếp lại dãy và chọn ra phần tử giữa vốn có độ phức tạp $O(N*Log(N))$. Thêm nữa, cho dù vẫn có các thuật toán tìm trung vị không-random với độ phức tạp $O(N)$, thuật toán này vẫn được áp dụng nhiều hơn nhờ cài đặt giản và tốc độ nhanh hơn.
