...
May thay, có nhiều thuật toán sắp xếp khác hiệu quả hơn như [Quick Sort](https://en.wikipedia.org/wiki/Quicksort), [Heap Sort](https://en.wikipedia.org/wiki/Heapsort), [Merge Sort](https://en.wikipedia.org/wiki/Merge_sort),... Nhiều thuật có độ phức tạp chỉ $O(N\*Log(N))$. Độ phức tạp nhỏ hơn rút ngắn rất nhiều thời gian để sắp xếp 1 tỉ con số, đến mức một chiếc máy tính bình thường nhất cũng thực hiện được. Thay vì $10^{18}$ câu lệnh, máy chỉ phải thực hiện $10^{10}$.
