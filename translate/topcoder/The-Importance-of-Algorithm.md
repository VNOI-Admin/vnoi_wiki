...
Một trong những thuật toán nhanh nhất để giải quyết vấn đề này có độ phức tạp $O(E \* V \* Log(N))$, với $E$ là số đường, $V$ là số giao lộ. Để dễ hình dung, thuật toán sẽ mất 2 giây để tìm đường đi ngắn nhất trong thành phố có 10000 giao lộ và 20000 con đường. Thuật toán này - **[Djikstra](https://en.wikipedia.org/wiki/Dijkstra%27s_algorithm)** - khá phức tạp và đòi hỏi sự vận dụng cấu trúc dữ liệu **[Hàng đợi ưu tiên (Priority Queue)](https://en.wikipedia.org/wiki/Priority_queue)**. Tuy nhiên trong trường hợp ngay cả **Djisktra** cũng trở nên quá chậm (tưởng tượng đường đi ngắn nhất từ Bắc Giang đến Cà Mau - có đến hàng triệu giao lộ), các lập trình viên sẽ sử dụng **heuristics**. Heuristics là các cách giải quyết vấn đề nhanh hơn bằng cách sử dụng những "suy đoán hợp lí" đặc thù cho vấn đề, với cái giá là kết quả không tối ưu (lắm). Chẳng hạn, một số thuật toán có thể chạy nhanh hơn **Djisktra** trong một số trường hợp - như **[thuật toán A\*](https://en.wikipedia.org/wiki/A*_search_algorithm) ** - tuy nhiên yêu cầu khoảng cách giữa 2 điểm. Vậy ta có thể sử dụng heuristics, tìm gần đúng giá trị khoảng cách và từ đó cải thiện tốc độ, thay vì trông chờ vào **Djisktra** vốn đã "bó tay". Phương pháp này không phải lúc nào cũng rút ngắn thời gian chạy của thuật trong trường hợp tệ nhất, nhưng nó hiệu quả với phần lớn trường hợp thực tế.
...
Thực tế, có nhiều bài toán quan trọng mà thuật tối ưu của chúng quá chậm và không sử dụng được. Nhóm bài nổi tiếng nhất cho tính cách này được gọi là NP - non-deterministic polynomial (đừng quan tâm cái tên này làm gì). Khi một bài toán nào đó được cho là NP-hard hay NP-complete, đồng nghĩa không ai biết thuật toán tối ưu của nó. Hơn nữa, nếu ai đó nghĩ ra được thuật tối ưu cho một vấn đề NP-complete, thuật đó cũng áp dụng được cho mọi vấn đề NP-complete.
...
Một cách tiếp cận vấn đề khác đó là thử "randomize" thuật toán theo một cách nào đó. Tuy không cải thiện tốc độ trong trường hợp xấu nhất, biện pháp này sẽ có hiệu quả với những trường hợp bình thường. **[QuickSort](https://en.wikipedia.org/wiki/Quicksort)** là một ví dụ điển hình cho việc randomize như vậy. Khi "suy biến", QuickSort sẽ sắp xếp một dãy trong $O(N)$, với $N$ là số phần tử của dãy. Nhưng nếu áp dụng random vào thuật toán, xác suất xảy ra suy biến là cực kì thấp, và độ phức tạp trung bình của QuickSort trở thành $O(N*Log(N))$. Có những thuật có độ phức tạp $O(N*Log(N))$ kể cả ở trường hợp xấu, nhưng chúng sẽ chậm hơn ở trường hợp bình thường. Dù cùng là $O(N*Log(N))$, QuickSort có hằng số $C$ nhỏ hơn, tức số lệnh cần thực hiện nhỏ hơn - $C*N*Log(N)$ - so với   (thường là) ($2*C*N*Log(N))$ của các thuật toán khác.

Một thuật toán khác dùng randomize để tìm trung vị của một dãy với độ phức tạp trung bình $O(N)$. Đây là cải tiến rất lớn so với việc sắp xếp lại dãy và chọn ra phần tử giữa vốn có độ phức tạp $O(N*Log(N))$. Thêm nữa, cho dù vẫn có các thuật toán tìm trung vị không-random với độ phức tạp $O(N)$, thuật toán này vẫn thu hút hơn nhờ sự đơn giản cùng tốc độ nhanh hơn.
...
Một lớp khác của thuật toán sẽ xử lí những vấn đề như **[data compression](https://en.wikipedia.org/wiki/Data_compression)** (nén dữ liệu). Loại thuật toán này không có output xác định mà thay vào đó cố gắng tối ưu ở những tiêu chí khác. Với data compression, thuật (như *LZW*) sẽ cố gắng làm dữ liệu sử dụng ít bộ nhớ nhất có thể, và vẫn có khả năng phục hồi lại trạng thái ban đầu. Số nhiều trường hợp thuật toán kiểu này sẽ dùng những kĩ thuật giống như các thuật toán bình thường, cố cho ra những output tốt, hay vừa đủ tốt. Ví dụ thuật toán của JPG và MP3, dù khiến data bị giảm chất lượng đi đôi chút, sẽ tạo ra những files nhỏ hơn files gốc rất nhiều. Files MP3 không cố gắng giữ lại mọi âm thanh của bài hát mà ghi vừa đủ thông tin để chất lượng vừa tốt, dung lượng files lại nhỏ. Files JPG cũng đi theo định hướng tương tự, dù các chi tiết bên trong khác xa.

## Tầm quan trọng của thuật toán ##

Với một lập trình viên, hay một nhà khoa học máy tính, hiểu rõ về các thuật toán vừa nêu là rất quan trọng để áp dụng thực tiễn. Nếu ta viết một phần mềm, ta sẽ phải đánh giá được phần mềm đó sẽ hoạt động nhanh chậm ra sao. Những đánh giá như vậy sẽ kém chính xác hơn nhiều nếu ta không có hiểu biết về runtime hay độ phức tạp. Thêm nữa, hiểu biết về thuật toán của những gì ta đang làm sẽ giúp ta dự đoán những trường hợp đặc biệt khiến phần mềm chạy chậm đi hay xảy ra lỗi.
...
Có rất nhiều ví dụ cho thấy các vấn đề thực tế đòi hỏi hiểu biết về thuật toán. Gần như mọi thứ bạn đang làm với máy tính được dựa trên một thuật toán nào đó mà có người phải rất vất vả mới tìm hiểu ra. Cho dù là ứng dụng đơn giản nhất của máy tính hiện đại cũng cần đến thuật toán để quản lí dữ liệu và truy xuất thông từ từ hard drive (ổ cứng).

Dù đời sống có rất nhiều ví dụ, tôi sẽ giới thiệu ở đây 2 bài toán thực tế có cùng trình độ với các bài ở TopCoder. Bài đầu là **[Maximum Flow Problem (Luồng Cực Đại)](https://en.wikipedia.org/wiki/Maximum_flow_problem)**, và bài sau có liên quan đến **[Dynamic Programming (Qui Hoạch Động)](https://en.wikipedia.org/wiki/Dynamic_programming)** - một kĩ thuật giải quyết được những vấn đề tưởng như không thể với tốc độ "điện giật".

## Maximum Flow - Luồng cực đại 
Bài toán luồng cực đại phải chỉ ra con đường tốt nhất để đưa một vật từ một điểm sang một điểm, thông qua một mạng lưới. Bài toán lần đầu được đặt những năm 1950 với mối quan tâm là mạng lưới đường sắt của Liên Xô. Mỹ muốn biết Liên Xô có thể tiếp tế cho các nước vệ tinh ở Đông Âu thông qua mạng lưới đường sắt nhanh như thế nào.
...
Thuật toán hiệu quả đầu tiên cho bài toán tìm luồng cực đại được phát triển bởi Ford và Fulkerson; thuật toán được đặt tên 2 nhà khoa học máy tính này và đã trở thành một trong những thuật toán nổi tiếng nhất ngành. Trong 50 năm qua, một số cải tiến đã được áp dụng để thuật nhanh hơn, nhiều khi cực kì phức tạp.

Từ lúc vấn đề ra đời, có nhiều ứng dụng đã được phát hiện. Thuật toán dĩ nhiên có chỗ đứng ở Internet, nơi lấy được thật nhiều data từ một điểm đến một điểm là rất quan trọng. Vấn đề cũng được ứng dụng trong kinh doanh, và là một phần quan trọng trong **nghiên cứu vận hành (operations research)**. Ví dụ, nếu bạn có N nhân viên và N công việc cần làm, nhưng không phải ai cũng làm được mọi thứ, thuật toán tìm luồng cực đại sẽ chỉ ra cách giao việc để các công việc đều hoàn thành, miễn là có thể thực hiện được.


## Sequence Comparison - So sánh chuỗi

Nhiều coder đi làm cả đời mà không từng phải cài một thuật toán qui hoạch động nào. Thế nhưng qui hoạch động là cần thiết cho nhiều thuật toán quan trọng. Một thuật có lẽ nhiều người đã từng sử dụng qua, dù không nhớ, đó là tìm điểm khác biệt của 2 chuỗi. Cụ thể hơn, tính toán xem sau bao nhiêu lần thêm, xóa hay sửa thì xâu A sẽ trở thành xâu B.

Ví dụ, có 2 xâu "AABAA" và "AAAB". Để chuyển xâu đầu thành xâu sau, đơn giản nhất là xóa kí tự 'B' ở giữa rồi chuyển 'A' cuối xâu thành 'B'. Thuật toán này có nhiều ứng dụng, có thể kể đến các vấn đề liên quan đến DNA hay chống đạo văn. Thế nhưng, hình thức áp dụng phổ biến của các lập trình viên là so sánh 2 phiên bản source code của cùng 1 file. Nếu các phần tử của chuỗi là các dòng của file, thuật sẽ cho ta biết dòng code nào bị xóa, dòng nào bị thêm vào hay sửa đi trong các phiên bản đó.

Không có qui hoạch động, ta sẽ phải xét các trường hợp theo cấp số mũ để biến một xâu thành xâu khác. Tuy vậy qui hoạch động cũng chỉ hoạt động với độ phức tạp $O(N*M)$, với N và M là số phần tử của mỗi xâu.

## Kết luận

Số thuật toán khác nhau mà con người học cũng nhiều như số bài toán khác nhau mà ta giải. Thế nhưng khả năng cao là vấn đề bạn đang cố gắng giải có liên quan đến một vấn đề khác theo một mặt nào đó. Có một vốn hiểu biết sâu và rộng về các thuật toán sẽ giúp bạn chọn lựa được hướng đi đúng và áp dụng thành công. Nhiều vấn đề dù nhìn có vẻ không thực tế, đòi hỏi những kiến thức thuật toán ta vẫn sử dụng hằng ngày.