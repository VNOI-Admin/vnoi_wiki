...
Gọi $f, g$ là các hàm số dương không giảm trên tập số nguyên dương (lưu ý rằng *hàm thời gian chạy* thỏa mãn điều kiện này). Ta nói rằng "$f(N)$ thuộc $\mathcal{O}(g(N))$" (cách đọc: "$f$ thuộc O-lớn của $g$") nếu tồn tại các giá trị $c$ và $N_0$ thỏa mãn điều kiện sau:

$$
\forall N > N_0; f(N) < c \times g(N)
$$

Mệnh đề trên có thể diễn dịch như sau: $f(N)$ thuộc $\mathcal{O}(g(N))$ nếu với $c$ nào đó toàn bộ đồ thị của hàm $f$ nằm dưới đồ thị của hàm $c \times g$. Chú ý rằng điều này có nghĩa là tốc độ tăng của hàm $f$ không vượt quá độ tăng của hàm $g$. (ND: ký hiệu O-lớn là ký hiệu chỉ **tập hợp của các hàm số**, vì vậy ở đây quan hệ giữa $f$ và $\mathcal{O}(g(N))$ là *(phần tử) thuộc (tập hợp)*.)

Thay vì viết "$f(N)$ thuộc $\mathcal{O}(g(N))$" ta thường viết là "$f(N)$ = $\mathcal{O}(g(N))$". Chú ý là dấu "=" không có tính đối xứng - viết "$\mathcal{O}(g(N)) = f(N)$" là sai và không có ý nghĩa gì, đồng thời mệnh đề "$g(N) = \mathcal{O}(f(N))$" cũng không phải luôn đúng (sẽ được chỉ ra ở phần sau của bài viết).

Định nghĩa trên được biết tới là ký pháp O-lớn và được sử dụng để chỉ ra cận trên của tốc độ tăng của một hàm số.

Xét hàm số $f(N) = 1.5N^{2} - 0.5N$ trong ví dụ 2. Ta có thể phát biểu rằng $f(N) = \mathcal{O}(N^{2})$ (một trường hợp khả dĩ cho các hằng số là $c = 2$ và $N_0 = 0$). Điều này có nghĩa là hàm $f$ **không tăng (tiệm cận) nhanh hơn** $N^{2}$.
...
Việc xác định được cận trên $\mathcal{O}(N^{2})$ của $f(N)$ cũng đưa ta tới cùng nhận định như trên - đảm bảo rằng độ tăng của hàm thời gian chạy tối đa là hàm bậc hai.

Vì vậy, chúng ta sẽ sử dụng ký pháp O-lớn để mô tả độ phức tạp *thời gian* (và đôi khi là cả *bộ nhớ*) của các thuật toán. Với thuật toán trong ví dụ 2 ta sẽ nói "Độ phức tạp thời gian của thuật toán này là $\mathcal{O}(N^{2})$" hoặc ngắn gọn hơn "Thuật toán này là $\mathcal{O}(N^{2})$".

Theo cách tương tự ta sẽ định nghĩa $\Omega$ (Omega-lớn) and $\Theta$ (Theta-lớn).

Ta nói rằng $f(N) = \Omega(g(N))$ nếu $g(N) = \mathcal{O}(f(N))$, nói cách khác nếu $f$ tăng nhanh hơn hoặc bằng $g$.

Ta nói rằng $f(N) = \Theta(g(N))$ nếu $f(N) = \mathcal{O}(g(N))$ và $g(N) = \mathcal{O}(f(N))$, nói cách khác nếu cả hai hàm số có độ hiệu quả xấp xỉ bằng nhau.
...
+ $1.5N^{2} - 0.5N = \mathcal{O}(N^{2})$.
+ $47N \log N = \mathcal{O}(N^{2})$.
+ $N \log N + 1000047N = \Theta(N \log N)$.
+ Tất cả các đa thức bậc $k$ là $\mathcal{O}(N^{k})$.
+ Độ phức tạp thời gian của thuật toán trong ví dụ 2 là $\Theta(N^{2})$.
+ Nếu một thuật toán thuộc $\mathcal{O}(N^{2})$, nó cũng thuộc $\mathcal{O}(N^{5})$.
...
+ Hàm số cho biết số răng của một người ở một thời điểm xác định là $\mathcal{O}(1)$.
+ Một thuật toán quay lui đơn giản giải các bài toán trên bàn cờ vua là $\mathcal{O}(1)$ vì cây vị trí mà thuật toán duyệt qua có kích cỡ giới hạn. (Tuy nhiên giá trị hằng số trong $\mathcal{O}(1)$ này lại rất lớn)
+ Mệnh đề "Độ phức tạp thời gian của thuật toán này tối thiểu là $\mathcal{O}(N^{2})$" là vô nghĩa. (Diễn dịch: "Giá trị tối thiểu của độ phức tạp thời gian của thuật toán này tối đa là xấp xỉ hàm bậc hai"). Phát biểu đúng là: "Độ phức tạp thời gian của thuật toán này là $\Omega(N^{2})$."
...
Với hầu hết các thuật toán thường gặp trong thực tế, giá trị hằng số của $\mathcal{O}$ (hoặc $\Theta$) thường là khá nhỏ. Nếu một thuật toán là $\Theta(N^{2})$, độ phức tạp chính xác là vào khoảng $10N^{2}$ chứ không phải $10^{7}N^{2}$.
...
Thông thường khi trình bày một thuật toán, cách tốt nhất để đánh giá độ phức tạp của nó là ký pháp Theta $\Theta$. Tuy nhiên, trong thực tế thường ta chỉ trình bày cận $\mathcal{O}$-lớn vì ký pháp này dễ viết hơn và phổ biến hơn. Nhắc lại rằng $\mathcal{O}$-lớn chỉ mang ý nghĩa cận trên. Thông thường ta tìm cận trên $\mathcal{O}$-lớn nhỏ nhất có thể.
...
Thoạt nhìn ta có thể sẽ đánh giá thuật toán trên có cận $\mathcal{O}(N^{2})$ - vòng lặp trong cùng chạy $N$ lần, mỗi lần tăng biến $j$ tối đa $N$ lần. Phân tích kỹ hơn, ta thấy rằng cận trên có đánh giá tốt hơn là $\mathcal{O}(N)$ - thật ra tổng thể số lần chạy câu lệnh $j$++ trong toàn bộ thuật toán không vượt quá $N$ lần.

Mệnh đề "thuật toán trên là $\mathcal{O}(N^{2})$" vẫn là một mệnh đề đúng. Tuy nhiên bằng việc nói rằng "thuật toán trên là $\mathcal{O}(N)$" ta cung cấp nhiều thông tin hơn về thuật toán đó.
