...
* **Độ phức tạp:**
   ...
   * Ở cách xây dựng thứ 2 ta thấy nếu một nút ở độ xâu $k$ nó sẽ mất không quá $h - k$ lần so sánh với nút con (nhắc lại $h$ là độ sâu của cây và số nút của cây $N < 2^{h+1}$) và một nửa số nút trên cây là lá và sẽ không phải so sánh với nút con nào cả, ta suy ra được:
      * Ở độ sâu $k = h-1$ có $2^{h-1}$ nút số phép so sánh là $1 \times 2^{h-1} = 1/4 \times N$
      * Ở độ sâu $k = h-2$ có $2^{h-2}$ nút số phép so sánh là $2 \times 2^{h-2} = 2/8 \times N$
      * Ở độ sâu $k = h-3$ có $2^{h-3}$ nút số phép so sánh là $3 \times 2^{h-3} = 3/16 \times N$
        ...
      * Ở độ sâu $k = 0$ (gốc) có $1$ nút số phép so sánh là $h \times 1 = h/N \times N$
      * Tổng hợp lại ta có: $1/4 \times N + 2/8 \times N + 3/16 \times N + ... + h/N \times N = N$ phép so sánh
