...
Với mỗi lần tìm điểm tiếp theo, ta cần duyệt qua tất cả các điểm trong tập, vì vậy độ phức tạp của mỗi lần tìm điểm là $\mathcal{O}(n)$ với $n$ là số lượng điểm trong tập. Số lần tìm điểm tiếp theo phụ thuộc vào số lượng điểm là đỉnh của bao lồi, gọi số lượng điểm đó là $h$, vì vậy độ phức tạp của cả thuật toán là $\mathcal{O}(nh)$. Trong trường hợp xấu nhất, $h=n$ hay tất cả các điểm trong dữ liệu vào tạo thành một đa giác lồi, độ phức tạp của thuật toán là $\mathcal{O}(n^{2})$, không đủ nhanh khi $n > 5000$.
...
Để đảm bảo ta có thể loại bỏ điểm $H_1$ với độ phức tạp $\mathcal{O}(1)$ (tức là điểm $H_2$ cũ trở thành điểm $H_1$ mới,...), ta nên dùng cấu trúc dữ liệu [hàng đợi hai đầu](http://vnoi.info/contributor/algo/data-structures/data-structures-overview#1-ctdl-l%C6%B0u-tr%E1%BB%AF_1-2-stack-queue-deque) (deque) hoặc [danh sách liên kết](http://vnoi.info/contributor/algo/data-structures/data-structures-overview#1-ctdl-l%C6%B0u-tr%E1%BB%AF_1-1-m%E1%BA%A3ng-array-danh-s%C3%A1ch-li%C3%AAn-k%E1%BA%BFt-linked-list) (linked list).

Về độ phức tạp của thuật toán, ta thấy bước sắp xếp các điểm có độ phức tạp $\mathcal{O}(n \log{n})$. Mỗi điểm được cho vào bao nhiều nhất một lần nên tổng độ phức tạp của các bước thêm điểm là $\mathcal{O}(n)$, và mỗi điểm bị loại ra khỏi bao nhiều nhất một lần nên tổng độ phức tạp của các bước xóa điểm là $\mathcal{O}(n)$, do đó độ phức tạp của bước xét các điểm là $\mathcal{O}(n)$. Vậy, độ phức tạp của thuật toán Graham là $\mathcal{O}(n \log{n})$, phù hợp cho hầu hết các bài toán. 
