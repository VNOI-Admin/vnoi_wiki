...
$$
P(\bigcup_{i=1}^{n} A_i) = \sum_{i=1}^{n} P(A_i) - \sum_{i \ne j} P(A_i A_j) + P(A_1 A_2 A_3) + P(A_1 A_2 A_4) + \cdots + P(A_{n-2} A_{n-1} A_n) - \cdots - (-1)^{n} \times P(A_1 A_2 \cdots A_n)
$$

Nếu gọi $B$ là tập hợp các tập hợp $A_i$, công thức này cũng có thể viết gọn như sau:

$$
P(\bigcup_{i=1}^{n} A_i) = \sum_{C \subset B} (-1)^{\|C\|-1} \times P(\bigcap_{e \in C} e)
$$
...
Vì vậy, số lần $x$ được đếm là $T = \binom{k}{1} - \binom{k}{2} + \binom{k}{3} - \cdots + (-1)^{i-1} \times \binom{k}{i} + \cdots + (-1)^{k-1} \times \binom{k}{k}$.

Để tính $T$, ta khai triển $(1-x)^{k}$ bằng **nhị thức Niu-tơn (Newton binomial)**:

$$
(1-x)^{k} = \binom{k}{0} - \binom{k}{1} \times x + \binom{k}{2} \times x^{2} - \binom{k}{3} \times x^{3} + \cdots + (-1)^{k} \times \binom{k}{k} \times x^{k}
$$
...
Ta có thể tính số số trong đoạn $[1;r]$ chia hết cho $p_i$ bằng công thức $\left\lfloor \frac{r}{p_i} \right\rfloor$.
