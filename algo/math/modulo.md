...
- **Bội chung nhỏ nhất** ($\operatorname{lcm}(a, b)$ hoặc $[a, b]$): Số nguyên dương $m$ nhỏ nhất thoả mãn $a\mid m$ và $b\mid m$
...
$$
\forall i\in\overline{1 \ldots n}, a_i\equiv b_i\pmod{m}\Rightarrow \sum\limits_{i = 1}^{n}a_i \equiv \sum\limits_{i = 1}^{n}b_i\pmod{m}
$$
...
$$
\forall i\in\overline{1 \ldots n}, a_i\equiv b_i\pmod{m}\Rightarrow \prod\limits_{i = 1}^{n}a_i \equiv \prod\limits_{i = 1}^{n}b_i\pmod{m}
$$
...
$$
\forall i\in\overline{1 \ldots n}, a\equiv b\pmod{m_i} \Rightarrow a\equiv b\pmod{[m_1, m_2, \ldots, m_n]}
$$
...
**Nghịch đảo modulo**: Số tự nhiên $\gamma$ được gọi là [**nghịch đảo modulo**](/algo/math/modular-inverse.md) theo modulo $m$ của một số tự nhiên $a$ nếu $a\gamma \equiv 1 \pmod{m}$. Ký hiệu là $a^{-1} \pmod{m}$.
...
[Hàm phi Euler](/translate/he/Number-Theory-4.md) của số tự nhiên $x$, ký hiệu $\varphi(x)$, là số lượng số tự nhiên nhỏ hơn hoặc bằng $x$ mà nguyên tố cùng nhau với $x$
$$
\varphi(x) = |\{k\in\mathbb{N}^{*}, k\leq x\mid (x,k)=1\}|
$$
Nếu $N$ phân tích được thành thừa số nguyên tố dưới dạng $N=p_1^{\alpha_1}p_2^{\alpha_2}\cdots p_m^{\alpha_m}$ thì ta có thể tính $\varphi(N)$ như sau:
$$
\varphi(N) = N\prod_{i = 1}^{m}\left(1-\frac{1}{p_i}\right)
$$

**Định lý Euler**: Nếu $a$ và $m$ là các số tự nhiên thoả mãn $(a, m) = 1$ thì ta có $a^{\varphi(m)}\equiv 1\pmod{m}$

**Định lý Fermat nhỏ**: Với $p$ là số nguyên tố bất kỳ và $n$ là số tự nhiên bất kỳ thoả mãn $(n, p) = 1$, ta có $n^{p-1} \equiv 1 \pmod{p}$. Đây là trường hợp đặc biệt của định lý Euler đối với số nguyên tố, khi đó $\varphi(p) = p-1$.
...
**Định lý**: Cho $2n$ số nguyên dương $a_1, a_2, \ldots, a_n$ và $m_1, m_2, \ldots, m_n$ sao cho các số $m_1, m_2, \ldots, m_m$ nguyên tố cùng nhau đôi một. Hệ phương trình đồng dư (hoặc thặng dư) ẩn $x$:
$$
\begin{cases}x\equiv a_1\pmod{m_1}\\x\equiv a_2\pmod{m_2}\\\ldots\\x\equiv a_n\pmod{m_n}\end{cases}
$$
có họ nghiệm duy nhất  là:
$$
x \equiv \sum_{i = 1}^{n}p_ip_i^{-1}a_i\pmod{M}
$$
trong đó $M = \prod\limits_{i = 1}^{n}m_i$, $p_i=\frac{M}{m_i}$ và $p_i^{-1}$ là nghịch đảo modulo $m_i$ của $p_i$.

:::spoiler **Chứng minh**
_Chứng minh sự tồn tại_: Do $m_1, m_2, \ldots, m_n$ là các số đôi một nguyên tố cùng nhau nên dễ thấy $(p_i, m_i) = 1$, dẫn đến sự tồn tại của $p_i^{-1}\pmod{m_i}$. Vì $p_ip_i^{-1}\equiv 1\pmod{m_i}$ nên
$$
p_ip_i^{-1}a_i\equiv a_i\pmod{m_i}
$$
Xét $x = \sum\limits_{i = 1}^{n}p_ip_i^{-1}a_i$. Ta thấy với mọi $i\neq j$ thì $m_i\mid p_j$ do $p_j$ là tích của tất cả các $m_{i'}$ với ${i'}$ khác $j$. Vì vậy với mọi $i$ ta có
$$
...
$$
x\equiv y\pmod{[m_1, m_2, \ldots, m_n]}
$$
Do $(m_1, m_2, \ldots, m_n) = 1$ nên
$$
