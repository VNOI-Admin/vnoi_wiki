...
Ở đây, xin giới thiệu với bạn đọc **định lý Wilson**:
    ...
    $$
Nếu chuẩn bị trước $0!,~ 1!,~ 2!,~ \dots,~ (p-1)!$ ta có thể tính $n!_p$ trong thời gian $\mathcal{O}(1)$ và $n!_{p}$ trong thời gian $\mathcal{O}(\log_{p} n)$.
...
- Chuẩn bị: $\mathcal{O}(N \log\log N)$
- Truy vấn: $\mathcal{O}(N)$
...
- *Ý tưởng:*
    - Chia đoạn $[1;p]$ thành các đoạn riêng biệt có kích thước $S$, ta sẽ có tổng cộng $\texttt{CNT} = \left\lfloor \frac{p}{S} \right\rfloor + 1$ đoạn.
    - Ta sẽ tính sẵn một mảng gồm $\texttt{CNT}$ giá trị sau: $0!,\, S!,\, (2S)!,\, \ldots,\, \left(\left\lfloor \frac{p}{S} \right\rfloor \cdot S\right)!$ với độ phức tạp $\mathcal{O}(p)$. Điều này không có ý nghĩa lắm. Vì thế, ta chỉ cần chạy code ra chương trình khác rồi lấy kết quả vào bài, bước này sẽ chỉ còn độ phức tạp $\mathcal{O}\left(\frac{p}{S}\right)$ nhưng lại tăng độ dài của code!
...
- *Code C++ minh họa:*
    ...
    Độ phức tạp phần chuẩn bị này là :$\mathcal{O}(p)$.
    - Phần code chính:
    ...
    ```
- *Đánh giá*:
    - Độ phức tạp không gian: $\mathcal{O}\left(\frac{p}{S}\right)$
    - Độ phức tạp thời gian:
        - Tiền xử lý (để lưu mảng `fact[]`): $\mathcal{O}\left(\frac{p}{S}\right)$
        - Truy vấn $\mathcal{O}\left(S\right)$
...
Đây là thuật toán nhân hai đa thức bậc không vượt quá $n$ với độ phức tạp $\mathcal{O}(n \log n)$.
...
- *Ý tưởng:* Ta sẽ tính $n! \bmod p$ như sau:
    ...
    Khi này:
    $$
    n! = \underbrace{P(0) \cdot P(m) \cdot P(2m) \ldots P\Big( (m-1)m \Big)}_{{\large\text{chính là }} 1\cdot 2\cdot 3 \ldots m^{2}} \cdot \underbrace{(m^{2} + 1)\ldots n}_{{\large\text{tính được trong }} \mathcal{O}\left(\sqrt n\right)} \pmod{p}
    $$
...
- Sau đó, ta tiếp tục sử dụng **FFT/NTT** để tính giá trị $P(x) \bmod M$ tại $m$ điểm: $k\cdot m$ với $k = 0, \ldots, (m - 1)$. Đó cũng chính là bài toán [POLYEVAL - Codechef](https://discuss.codechef.com/t/polyeval-editorial/12771) hoặc [Library Checker - Multipoint Evaluation](https://judge.yosupo.jp/problem/multipoint_evaluation).
- Chú ý rằng [POLYEVAL - Codechef](https://discuss.codechef.com/t/polyeval-editorial/12771) là bài toán dễ hơn, và bạn có thể sử dụng chia để trị với không gian lưu trữ $\mathcal{O}(p)$. Để làm được bài [Library Checker - Multipoint Evaluation](https://judge.yosupo.jp/problem/multipoint_evaluation), ta sử dụng thuật toán chia để trị và chia đa thức để đạt được không gian lưu trữ $\mathcal{O}\left(\sqrt p\right)$:
...
$$
\begin{align*}
P_0(x) &= P(x) \bmod \Big((x-x_1)(x-x_2)\cdots(x-x_{\lfloor n/2 \rfloor})\Big)\\
P_1(x) &= P(x) \bmod \Big((x-x_{n/2+1})\cdots(x-x_n)\Big)
\end{align*}
$$
$$
\implies\begin{align*}
...
- *Đánh giá:*
    - Độ phức tạp không gian: $\mathcal{O}\left(\sqrt p\right)$
    - Độ phức tạp thời gian: $\mathcal{O}\left(\sqrt p \log^{2} p\right)$
...
- ***Đánh giá:***
    - Độ phức tạp không gian: $\mathcal{O}(\sqrt n)$
    - Độ phức tạp thời gian: 
        - Hàm Lagrange ở phần sử dụng sử dụng FFT có độ phức tạp $\mathcal{O}(N \log N)$ với $N = \deg(h) + \texttt{cnt}$.
        Các vòng lặp khác chỉ có độ phức tạp tối đa đến $\mathcal{O}(N)$.
        - Hàm factorial có độ phức tạp lớn nhất ở phần sử dụng hàm Lagrange với tổng cộng: $\mathcal{O}\left(s\log s + \frac{s}{2}\log\frac{s}{2} + \frac{s}{4}\log\frac{s}{4} + \ldots \right)$ với $s = \lfloor \sqrt n \rfloor$. Chú ý rằng:
          $$
          \begin{align}
          ...
          \end{align}
          $$
        - Điều này có nghĩa là bài toán có tổng độ phức tạp: $\mathcal{O}\left(\sqrt n \log n\right)$
