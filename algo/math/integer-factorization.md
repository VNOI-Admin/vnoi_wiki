...
- **TH2:** Ở vòng lặp trước ta không tìm được chu trình nào có độ dài $\le \texttt{power_prev} = 2^{i-1}$ nên phải tìm một chu kỳ có độ dài lớn hơn $\texttt{power_prev} = \frac{\texttt{power_now}}{2} = \frac{s}{2}$. Mà ở vòng lặp hiện tại, ta đã cố định $s = 2^{i}-1$, và cho $t$ chạy từ $s+1$ đến $2s+1$ nên ta bỏ qua $s < t < \frac{3s}{2}$.  -->
