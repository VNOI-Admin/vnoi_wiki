...
> **Tại sao lại như vậy?**
Giả sử tại một thời điểm, ta duyệt đến một số hợp $d$ có dạng $d = p_1 \times p_2 \times \cdots \times p_k$, trong đó $p_i$ là các thừa số nguyên tố. Khi $n$ chia hết cho $d$, điều này có nghĩa là $n$ cũng đã chia hết cho một trong các thừa số nguyên tố $p_i$. Các thừa số này sẽ được xử lý bằng phép chia `n /= d` trong quá trình duyệt các số nhỏ hơn $d$.
...
Để thuận tiện cho việc cài đặt, ta định nghĩa mảng $\texttt{increments}$ để nhảy đến số "nghi ngờ là số nguyên tố" tiếp theo.
...
> Do số nhân tử của $N$ tối đa là $\mathcal{O}(\log N)$, toàn bộ thuật toán phân tích $N$ bằng thuật Pollard's Rho có độ phức tạp trung bình là là $\mathcal{O} \left( \sqrt[4] N \log^{2} N \right)$. Do thuật toán sử dụng yếu tố ngẫu nhiên, cài đặt sử dụng Pollard's Rho sẽ không có độ phức tạp chính xác.
...
```
Chú ý rằng, không phải kỳ thi nào cũng có thể sử dụng `__int128_t` (do máy chấm) nên có thể sử dụng hàm mult thay thế sau ([nhân Ấn Độ](/algo/algebra/binary_exponentation.md#nh%C3%A2n-l%E1%BA%A5y-d%C6%B0) với độ phức tạp $\mathcal{O}(\log b)$):
```cpp=
long long mult(long long a, long long b, long long mod) {
    if (!b)
        return 0;
    long long x = mult(a, b / 2, mod);
    if (b % 2 == 0)
        return 2 * x % mod;
    else
        return (2 * x + a) % mod;
...
Gọi $\lambda$ là độ dài chu trình và $\mu$ là vị trí đầu tiên xuất hiện chu trình. Vẫn ý tưởng sử dụng hai con trỏ, nhưng ta sẽ tìm $\texttt{power} = 2^{k}$ nhỏ nhất mà lớn hơn cả $\lambda$ và $\mu$.
Sau đó, ta sẽ tìm $x_s, x_t$ bằng cách cho $s=\texttt{power}$ và tìm vị trí $t$ sao cho $x_s=x_t$.
...
- **TH1:** Ở vòng duyệt trước, Rùa ở vị trí chưa bắt đầu chu trình hay $\mu > \texttt{power}$. Khi này ta chỉ cần tăng $\texttt{power}$ lên là có thể tìm được chu trình. Giả sử việc bỏ qua $s < t < \frac{3s}{2}$ vô tình làm bỏ qua $t=s+\lambda$ thì suy ra $\lambda < \frac{s}{2}$ và suy ra $s+2\lambda < 2s$... Cứ tiếp tục làm như vậy ta có thể chắc chắn rằng tồn tại $\alpha$ để $\frac{3s}{2} \le s+2^\alpha \cdot \lambda < 2s$ và ta tìm được đây là vị trí của thỏ.
- **TH2:** Ở vòng lặp trước ta không tìm được chu trình nào có độ dài $\le \texttt{power\_prev} = 2^{i-1}$ nên phải tìm một chu kỳ có độ dài lớn hơn $\texttt{power\_prev} = \frac{\texttt{power\_now}}{2} = \frac{s}{2}$. Mà ở vòng lặp hiện tại, ta đã cố định $s = 2^{i}-1$, và cho $t$ chạy từ $s+1$ đến $2s+1$ nên ta bỏ qua $s < t < \frac{3s}{2}$.  -->
Khi duyệt đến $\texttt{power}=2^{i} > \max\{\mu, \lambda\}$, cho Rùa đứng ở vị trí $s=2^{i}-1$. Xét việc bỏ qua các giá trị $t$ mà $s < t < \frac{3s}{2}$.
...
Dãy số: $2, 5, 26, 105, 15, 83, 26, 105, \ldots$ Ở bước $i=2$, ta không xét $15$ do $s < t < \frac{3s}{2}$</small>
<center>
  
| $i$  | $s=2^{i}-1$ | $x_{s} \bmod N$ | $x_{2^{i}} \rightarrow x_{2^{i+1}-1} \bmod N$  | $\mid x_{t} - x_{s}\mid$  | $\gcd(\mid x_t - x_{s}\mid, N)$ |
|----|---------|-----------------|-----------------------------------------------|---------------------|---------------------------|
| $0$  | $0$       | $2$           | $5$                                             | $3$                   | -                         |
...
- Thời gian trung bình: $\mathcal{O}\left(\sqrt[4]N \log^{2} N\right)$.
- Không gian: $\mathcal{O}(\log N)$ (để lưu kết quả phân tích).
...
|       Lớn hơn $10^{100}$       | Sàng trường số tổng quát ([General number field Sieve](https://en.wikipedia.org/wiki/General_number_field_sieve)) (GNFS)                                                                                                                                                                                                                                                                    | $e^{\left(\sqrt[3]2 + \mathcal{O}(1)\right)(\log N)^{\frac{1}{3}} (\log\log N)^{\frac{2}{3}} }$.                                                                                               |
...
&nbsp; &nbsp; &nbsp; &nbsp; Ví dụ khi $\alpha=\frac{1}{2}, \beta=1$ và $N\sim 10^{100}$ thì độ phức tạp khoảng $\mathcal{O}(N^{0.154})$.
