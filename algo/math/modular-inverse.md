...
Với $\varphi(p)$ là [phi hàm Euler](/translate/he/Number-Theory-4) của $p$. Từ đây, ta suy ra: 
$$
a \times a^{\varphi(p) - 1} \equiv 1 \pmod{p}
$$

Vậy, nghịch đảo modulo sẽ bằng $a^{-1} = a^{\varphi(p) - 1} \bmod{p}$.
...
Việc tính phi hàm Euler với độ phức tạp $\mathcal{O}(\sqrt{m})$, cộng thêm với việc tính luỹ thừa với độ phức tạp $\mathcal{O}(\log{p})$, làm độ phức tạp của thuật toán bằng $\mathcal{O}(\sqrt{m} + \log{p})$. Nếu $m$ là số nguyên tố thì việc tính phi hàm Euler sẽ nhanh hơn (phi hàm Euler của một số nguyên số $p$ là $p - 1$), và độ phức tạp của thuật toán sẽ bằng $\mathcal{O}(\log{m})$.
...
int modInv(int a, int m) {
    int x, y;
    int d = extEuclid(a, m, x, y); // hàm euclid mở rộng ở bài Thuật toán Euclid
    if (d != 1)
...
Độ phức tạp của thuật toán bằng $\mathcal{O}(\log{a})$.
...
Khi duyệt từ $[1; m - 1]$, các giá trị $r^{-1}$ đã được tính trước do $r < a$.
...
Độ phức tạp của thuật toán này bằng $\mathcal{O}(m)$.

## Tính nhanh nghịch đảo modulo của một mảng

Để tính nghịch đảo modulo của một mảng, ta có thể tính bằng cách tìm nghịch đảo modulo của từng phần tử một (độ phức tạp $\mathcal{O}(n \log{m})$) hoặc tính nhanh nghịch đảo modulo trong đoạn $[1; m - 1]$ rồi tìm phần tử tương ứng (độ phức tạp $\mathcal{O}(m)$). Thay vì thế, ta chỉ cần thực hiện một lần tính nghịch đảo modulo  của tích tất cả các phần tử. Khi tìm nghịch đảo của một phần tử, ta chỉ cần nhân nó với các phần tử còn lại trong mảng.
...
Độ phức tạp của thuật toán bằng $\mathcal{O}(n + \log{m})$ với $n$ là kích thước mảng.
...
Độ phức tạp của thuật toán này cũng bằng $\mathcal{O}(n + \log{m})$.
