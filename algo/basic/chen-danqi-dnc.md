...
Bài toán trên có thể được giải bằng CDQ D&C với độ phức tạp $\mathcal{O}(n \log^{2}{n})$ với $n$ là số điểm.
## Ý Tưởng
### Các Bước Thực Hiện
...
Khi tăng số chiều của bài toán lên $3$, ta có thể dễ dàng xử lý bằng cách tăng chiều của Segment Tree/Fenwick Tree lên thành $2$ chiều để giải trong độ phức tạp $\mathcal{O}(n \log^{2}{n})$. Tuy nhiên, hệ số thời gian chạy của thuật toán này đôi lúc không đủ tốt, kèm theo độ phức tạp không gian lớn. Lúc này ta có thể áp dụng CDQ D&C để giải bài toán cũng trong $\mathcal{O}(n \log^{2}{n})$ nhưng có thời gian chạy và độ phức tạp không gian tốt hơn.

Các bước giải bằng CDQ trên tập điểm $3$ chiều có dạng như sau:

:::success
- Tiền xử lý: Sắp xếp tập $P$ theo chiều $x$.
- Chia: Xét đoạn $[l \ldots r]$ của tập $P$, ta chia đoạn này thành $2$ đoạn $[l \ldots m]$ và $[m+1 \ldots r]$ (lấy $m$ là vị trí chính giữa của đoạn).
- Đệ quy: Xử lý bài toán trên đoạn $[l \ldots m]$ và $[m+1 \ldots r]$.
- Kết hợp: Tính phần đóng góp của đoạn $[l \ldots m]$ cho đoạn $[m+1 \ldots r]$ và gộp hai đoạn sao cho kết quả **sắp xếp theo chiều $y$**.
:::

Khi đã sắp xếp tập $P$ theo chiều $x$, việc tính phần đóng góp của đoạn $[l \ldots m]$ cho đoạn $[m+1 \ldots r]$ lúc này được đơn giản hóa xuống bài toán gần giống bài toán hai chiều. Để tối ưu thuật toán "Kết hợp", sau khi thực hiện hàm chia để trị trên đoạn $[l \ldots r]$, ta cần đảm bảo đoạn được sắp xếp lại theo chiều $y$ (xem phần dưới).

#### Đánh Giá Độ Phức Tạp

- Tiền xử lý: Ở bước này ta sẽ thực hiện việc sort theo một chiều của bài toán, do đó độ phức tạp sẽ là $\mathcal{O}(n \log{n})$.
- $3$ bước còn lại có cách hoạt động giống với Merge Sort kết hợp cùng cấu trúc dữ liệu như Segment Tree hoặc Fenwick Tree. Do đó ở $3$ bước này sẽ có độ phức tạp là $\mathcal{O}(n \log{n} \times f)$ (với $f$ là độ phức tạp của cấu trúc dữ liệu được sử dụng) (để chứng minh độ phức tạp này ta có thể sử dụng Master Theorem: $T(n) = 2T\left(\frac{n}{2}\right) + \mathcal{O}(N \times f) = \mathcal{O}(N \log N \times f)$).
...
Khi này, đoạn $[l \ldots m]$ và $[m+1 \ldots r]$ đã được sắp xếp theo $y$. Do thao tác sắp xếp $2$ đoạn $[l \ldots m]$ và $[m+1 \ldots r]$ được thực hiện độc lập, nên mỗi phần tử trong đoạn $[m+1 \ldots r]$ vẫn có chiều $x$ lớn hơn hoặc bằng mọi phần tử trong đoạn $[l \ldots m]$.
...
Để tính được phần đóng góp của đoạn $[l \ldots m]$ cho đoạn $[m+1 \ldots r]$, ta sẽ duyệt qua cả $2$ đoạn tương tự Merge Sort theo chiều $y$ kết hợp cùng cấu trúc dữ liệu (ví dụ như Segment Tree). Cụ thể, ta so sánh hai phần tử đứng đầu hai đoạn và ưu tiên duyệt phần tử có chiều $y$ nhỏ hơn.

Khi duyệt qua phần tử thuộc $[l \ldots m]$, ta sẽ tiến hành cập nhật lên cấu trúc dữ liệu. Khi duyệt qua $1$ phần tử thuộc đoạn $[m+1 \ldots r]$, ta sẽ cập nhật đáp án cho phần tử này dựa theo cấu trúc dữ liệu.

Do ta chỉ cần tính đóng góp của đoạn $[l \ldots m]$ cho đoạn $[m+1 \ldots r]$ và chiều $x$ đã được sort từ ban đầu, chiều $y$ đã được sort qua việc thực hiện hàm `dnc()` trên $2$ đoạn, khi này từ $3$ chiều ta đã có thể rút gọn còn $1$ chiều $z$. 
...
void dnc(int l, int r) {
    ...
    for (int rptr = m + 1; rptr <= r; rptr++) {
        for (; lptr <= m && v[lptr].y <= v[rptr].y; lptr++) {
            // Cập nhật ctdl
            revert.push_back(v[lptr].z);
            tmp.push_back(v[lptr]);
        }
...
- Cuối cùng ta thực hiện CDQ tương tự như bài trên:
    ...
    ```
- Độ phức tạp: $\mathcal{O}((N+Q) \log^{2}{(N+Q)})$.
...
Nếu biết về thuật toán Centroid Decomposition, ta dễ dàng nhận thấy rằng $\frac{2}{3}$ chiều mà ta cần xét sẽ lần lượt là độ dài từ $u$ đến centroid và tổng trọng số từ $u$ đến centroid. Vậy chiều thứ $3$ của bài toán này là gì?
...
Độ phức tạp: $\mathcal{O}(N \log^{3}{N})$.
    
## CDQ Convolution - Online FFT
    
### Bài toán nền tảng
    
Ta cần tính dãy $c_0,c_1,c_2, \ldots, c_n$ sao cho $c_k = \sum_{i+j=k-1}{a_i b_j}$ nhưng ta sẽ chỉ biết được $a_k$ và $b_k$ sau khi ta tính được $c_k$.
...
Gọi $\texttt{dp}[i]$ là số cách di chuyển từ $(0, 0)$ đến $(i, 0)$. Nếu điểm hạ cánh trước đó là $i-j$, ta sẽ có $\operatorname{catalan}\left[\frac{j}{2} - 1\right]$ cách để đi từ $i-j$ đến $i$ và không hạ cánh ở giữa đường nếu $j$ chẵn, ngoài ra ta sẽ không có cách để đi từ $i-j$ đến $i$. Lúc này ta định nghĩa:
    
$$
\texttt{coef}[j] =
\begin{cases} 
\operatorname{catalan}\left(\frac{j}{2} - 1\right), & 2 \leq j \leq 2k \text{ và } j \text{ chẵn}, \\
0, & j \text{ lẻ hoặc } j \notin [2, 2k].
\end{cases}
$$
...
$$
\operatorname{sum}[i] = \texttt{dp}[0] + \cdots + \texttt{dp}[i].
$$
    
Ta có thể thấy lúc này ta có thể tính $\texttt{dp}[i]$ từ các giá trị $\operatorname{sum}[i-j]$ và $\texttt{coef}[j]$ với $0 \leq j \leq i$.
    
$$
\texttt{dp}[i] = \sum_{j=0}^{i} \operatorname{sum}[i - j] \cdot \texttt{coef}[j].
$$

Gọi $D(x)$, $C(x)$, $S(x)$ lần lượt là hàm sinh cho $\texttt{dp}$, $\texttt{coef}$ và $\operatorname{sum}$. Khi này ta có thể viết lại biểu thức trên thành:
...
Ta có: $g(x) = F(f(x))$ với $F( \ldots )$ là các hàm có thể đạo hàm được (differentiable). 
    
Khi này ta có thể viết $g(x)$ lại theo cách giống với thuật toán Newton:
    
$$
g(x) = F(f_0) + F'(f_0)(f - f_0) + \mathcal{O}((f - f_0)^{2}).
$$
