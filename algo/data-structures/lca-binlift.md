...
- Sau đó, ta cho $u$ và $v$ nhảy lên cha của chúng và lặp lại thao tác đến khi $2$ đỉnh này trùng nhau:
    - $u = \texttt{par}[u] = u_2, v = \texttt{par}[v] = v_2$
    - $u = \texttt{par}[u] = u_1, v = \texttt{par}[v] = v_1$
    - $u = \texttt{par}[u] = \texttt{lca}, v = \texttt{par}[v] = \texttt{lca}$
    - dừng thao tác vì $u$ và $v$ trùng nhau ($u = v = \texttt{lca}$)
...
## Bài toán 1
Cho một cây gồm $N$ đỉnh có gốc tại đỉnh $1$. Có $Q$ truy vấn, mỗi truy vấn gồm $1$ cặp số $(u, k)$, ta cần in ra tổ tiên thứ $k$ của $u$ (tổ tiên thứ $k$ của $u$ là $\texttt{par}[\texttt{par}[\ldots[u \underset{\color{blue}{k \text{ lần}}}{\color{blue}{\underbrace{\color{blue}{]\ldots]]}}}}$ ).
...
#### Phân tích:
- Độ phức tạp tiền xử lý: $\mathcal{O}(N)$ (tạo mảng $\texttt{par}$)
...
- Ta xây dựng mảng $\texttt{up2}[N]$ là tổ tiên thứ $2$ của mỗi đỉnh.
- Khi truy vấn, ta nhảy các bước độ dài $2$ trước, sau đó kiểm tra xem $k$ có $\geq 1$ hay không (vì $k \bmod 2$ dư $0$ hoặc $1$).
...
#### Phân tích:
- Độ phức tạp tiền xử lý: $\mathcal{O}(2N)$ (tạo mảng $\texttt{par}$ và $\texttt{up2}$)
...
- Độ phức tạp bộ nhớ: $\mathcal{O}(2N)$ (2 mảng $\texttt{par}$ và $\texttt{up2}$)
...
#### Phân tích:
- Độ phức tạp tiền xử lý: $\mathcal{O}(3N)$ (tạo mảng $\texttt{par}$, $\texttt{up2}$ và $\texttt{up4}$)
...
- Độ phức tạp bộ nhớ: $\mathcal{O}(3N)$ (3 mảng $\texttt{par}$, $\texttt{up2}$ và $\texttt{up4}$)
...
#### Phân tích:
- Độ phức tạp tiền xử lý: $\mathcal{O}(4N)$ (tạo mảng $\texttt{par}$, $\texttt{up2}$, $\texttt{up4}$ và $\texttt{up8}$)
...
- Độ phức tạp tiền xử lý: $\mathcal{O}(4N)$ (4 mảng $\texttt{par}$, $\texttt{up2}$, $\texttt{up4}$ và $\texttt{up8}$)

### Thuật toán tối ưu 2
Nếu ta làm tiếp như thuật toán tối ưu $1.3$ (tiếp tục tạo các mảng $\texttt{up16}, \texttt{up32}, \ldots, \texttt{up65536}$) ta sẽ có $\log_2(N)$ mảng up, độ phức tạp bài toán lúc này như sau:
- Độ phức tạp tiền xử lý: $\mathcal{O}(N \cdot (1 + \log_2(N))) = \mathcal{O}(N \cdot \log_2(N))$ (mảng $\texttt{par}$ và $\log_2$ mảng $\texttt{up}$)
...
- Độ phức tạp bộ nhớ: $\mathcal{O}(N \cdot \log_2(N))$ (mảng $\texttt{par}$ và $\log_2$ mảng $\texttt{up}$)

Nhưng nếu dùng $\log_2$ mảng $\texttt{up}$ sẽ mang đến cho ta nhiều bất tiện (code dài, dễ sai,...). Do đó, ta có thể đặt:
- $\texttt{up}[u][j]$ là tổ tiên thứ $2^{j}$ của $u$ (tương ứng $\texttt{up}_{2^{j}}[u]$).
    - Cha của $u$ là tổ tiên thứ $1$ (đầu tiên) của $u$.
        - $\texttt{up}[u][0] = \texttt{par}[u]$
    - Đặt $x$ là tổ tiên thứ $2^{j-1}$ của $u$, ta có, tổ tiên thứ $2^{j-1}$ của $x$ là tổ tiên thứ $2^{j}$ của $u$ (vì $2^{j-1} + 2^{j-1} = 2^{j}$)

$$\begin{cases}
x &= \texttt{up}[u][j - 1] \\
\texttt{up}[u][j] &= \texttt{up}[x][j - 1]
\end{cases}$$

- Ta có công thức truy hồi sau:

$$\begin{cases}
\texttt{up}[u][j] = \texttt{par}[u] & \text{ với } j = 0 \\
\texttt{up}[u][j] = \texttt{up}[\texttt{up}[u][j-1]][j-1] & \text{ với } j > 0 \text{ và } 2^{j} \leq h[u] \\
\texttt{up}[u][j] = 0 \text{ (}\texttt{NULL}\text{)} & \text{ với } j > 0 \text{ và } 2^{j} > h[u]
\end{cases}$$
...
Qua đó, ta có thể thấy rằng Binary Lifting chỉ đơn giản là một cách tiền xử lý dữ liệu nhằm giúp cho thời gian truy vấn tối ưu hơn. Về tính chất, Binary Lifting khá giống với chặt nhị phân, khác ở chỗ mỗi lần, Binary Lifting thì ta thử nhảy $2^{k}$ đơn vị, còn chặt nhị phân thì ta thử nhảy $\frac{\texttt{hi} - \texttt{lo} + 1}{2}$ đơn vị.
...
### Thuật toán 1
Ta xây dựng mảng $\texttt{dist}[u][j]$ là khoảng cách từ $u$ đến tổ tiên thứ $2^{j}$ của $u$.
...
#### Phân tích
- Độ phức tạp tiền xử lý: $\mathcal{O}(N\log N)$ (tạo mảng $\texttt{up}$ và $\texttt{dist}$)
...
### Thuật toán 2
Ta có nhận xét:
- Ta đã tính trước các khoảng cách có độ lớn $2^{j}$ (mảng $\texttt{dist}$)
...
Dễ thấy: nếu $x$ là tổ tiên chung của $u$ và $v$ ($x \neq$ gốc) thì $\texttt{par}[x]$ cũng là tổ tiên chung của $u$ và $v$. Do đó, ta có thể tìm tổ tiên chung gần nhất của $u$ và $v$ bằng Binary Lifting.

Bằng cách sử dụng mảng $\texttt{up}$, ta có thể nhảy từ $u$ đến bất kì tổ tiên nào chỉ trong $\mathcal{O}(\log N)$ (bài toán tìm tổ tiên thứ $k$). Ta có thể tính mảng này bằng hàm DFS như sau:
...
Cách tìm LCA giống hệt thuật toán ngây thơ 1, nhưng để tăng tốc, thay vì nhảy lên cha ở mỗi bước, ta dùng mảng $\texttt{up}$ để nhảy, từ đó thu được độ phức tạp $\mathcal{O}(\log N)$ cho mỗi truy vấn. Cụ thể:

- Gọi $h(u)$ là độ cao của đỉnh $u$. Để tính $\texttt{LCA}(u, v)$, giả sử $h(u) > h(v)$, đầu tiên ta tìm $u'$ là tổ tiên của $u$ và có $h(u') = h(v)$:
    - Rõ ràng, ta cần nhảy từ $u$ lên cha thứ $k = h(u) - h(v)$.

- Sau khi $u$ và $v$ đã ở cùng độ cao, ta sẽ tính $\texttt{LCA}(u, v)$:
    - Nếu $u = v$ thì $\texttt{LCA}(u, v)$ chính là $u$ và $v$.
...
Từ hai quan sát trên, thấy được chỉ cần ba giá trị $f(u)$, $f(v)$ và $f(p)$ để tính $\texttt{dist}(u, v)$. Khi cộng $f(u)$ và $f(v)$, các đỉnh thuộc phần giao bị tính đến 2 lần, vì vậy ta tính $\texttt{dist}(u, v) = f(u) + f(v) - 2 \times f(p)$.
...
- $!$ $\texttt{root}$ : Chọn $\texttt{root}$ làm gốc của cây.
- $?$ $u$ $v$ : Tìm tổ tiên chung gần nhất của $2$ đỉnh $u$ và $v$.

Giới hạn: $N, M \le 10^{5}$

## Thuật toán
- Xét cây có gốc là đỉnh bất kì (giả sử là đỉnh $1$), trong $3$ đỉnh $\texttt{lca}(u, v)$, $\texttt{lca}(u, \texttt{root})$, $\texttt{lca}(v, \texttt{root})$ sẽ luôn tồn tại ít nhất $2$ đỉnh trùng nhau, đỉnh còn lại chính là $\texttt{lca}(u, v)$ trong cây có gốc là $\texttt{root}$.
