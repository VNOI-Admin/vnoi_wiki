...
Tất nhiên trong quá trình thay một nút bằng nút tổ tiên của nó, ta sẽ sử dụng mảng $A$ để có thể nhảy một lần được nhiều bước. Khi đó độ phức tạp của thuật toán sẽ là $\left< O(N\log{N}), O(\log{N}) \right>$.
...
Từ 2 mảng $\text{prevnum}$ và $\text{postnum}$ ta có thể thấy điều kiện cần và đủ để $u$ là cha của $v$ là $\text{prevnum}[u] \le \text{prevnum}[v]$ và $\text{postnum}[u] \ge \text{postnum}[v]$. Do đó thao tác chất vấn $LCA(u,v)$ thực chất là tìm một đỉnh $i$ sao cho :

- $\text{prevnum}[i] \le \min(\text{prevnum}[u],\text{prevnum}[v])$
- $\text{postnum}[i] \ge \max(\text{postnum}[u],\text{postnum}[v])$
...
- Xét thủ tục `Find_LCA(left, right, u, v : Integer)` tìm cha chung gần nhất của $u$, $v$ trong mảng con $A[left..right]$. Không mất tính tổng quát giả sử $\text{prevnum}[u]<\text{prevnum}[v]$.

    - Nếu $\text{postnum}[u]>\text{postnum}[v]$ thì $LCA(u,v)=u$ và đây là trường hợp dễ dàng tìm ra đáp án.

    - Nếu $\text{postnum}[u]<\text{postnum}[v]$, gọi $mid = (left+right)/2$. Xét phần tử chính giữa đoạn $i = A[mid]$ sẽ có các khả năng sau:
        - $\text{postnum}[i]>\text{postnum}[v]$: $i$ sẽ là cha chung của $u$ và $v$ nhưng chưa chắc đã là $LCA(u,v)$. Hiển nhiên $\text{prevnum}[i] \le \text{prevnum}[LCA(u,v)]$ nên ta gọi đệ quy: `Find_LCA( mid, right, u, v)`.
        ...
        - $\text{postnum}[i]<\text{postnum}[u]$: đỉnh $i$ là đỉnh được rẽ nhánh ra từ một nút cha nào đó của $u$, nhưng ta hoàn toàn chưa biết nút cha này nằm dưới hay trên $LCA(u,v)$. Ta có thể xử lý theo 2 cách: gọi đệ quy `Find_LCA(left,right,cha(u),cha(v))` hoặc lấy `j = Find_LCA(left,mid,i,u)` và $j$ sẽ rơi vào 2 trường hợp đầu.

Thuật toán trên nếu chỉ thực hiện 2 trường hợp đầu thì độ phức tạp cho mỗi lần chất vấn là $\log N$, còn nếu chỉ thực hiện trường hợp 3 thì độ phức tạp sẽ là $N$. Qua khảo sát bằng việc chạy chương trình cho thấy thời gian thực hiện trung bình của thuật toán này ngang với các thuật toán với độ phức tạp $\left<O(N\log{N}),O(\log{N})\right>$. Thuật toán này tuy có độ phức tạp lớn nhưng lại là phương pháp tiết kiệm bộ nhớ và cài đặt dễ dàng nên đây là thuật toán có ứng dụng cao trong làm bài.
...
- Với một cây có $N$ đỉnh và nút gốc sẽ có các cây con với số đỉnh là $x_1, ..., x_k$. Giả sử $x_1 = \max(x_1..x_k)$. Ta có $2*\max(x_2..x_k) \le \max(x_2..x_k) + x_1 \le N$
    $\implies \max(x_2…x_k) \le N/2$.
...
- Theo cách xây dựng cây thì :
    ...
    - $F(x_2) +1  \le  F(N/2)+1  \le  \log N+1$
    - ...
    - $F(x_k) +1  \le  F(N/2)+1  \le  \log N+1$
...
Thuật toán này sẽ chạy trong thời gian $\left<O(N),O(\log{N})\right>$.
