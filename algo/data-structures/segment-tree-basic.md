...
- Ta có giải pháp [chia để trị](https://vi.wikipedia.org/wiki/Thu%E1%BA%ADt_to%C3%A1n_chia_%C4%91%E1%BB%83_tr%E1%BB%8B) sau:
    ...
    - Nếu không, ta chia dãy đó thành hai dãy con liên tiếp nhỏ hơn, mỗi dãy con gần bằng một nửa kích thước của dãy ban đầu, và tìm giá trị nhỏ nhất tương ứng của chúng. Giá trị nhỏ nhất của dãy ban đầu chính là giá trị nhỏ hơn giữa hai giá trị nhỏ nhất của các dãy con.
        ...
        ![](/uploads/algo/data-structures/segment-tree-basic/segment-tree-basic_img1.png)
...
- Nhận thấy rằng, khi ta sửa đổi giá trị của một phần tử bất kỳ trong mảng thì số lượng đoạn cần tính lại không nhiều. Vậy nên ta sẽ lưu lại kết quả và chỉ tính lại khi thực sự cần thiết.
    ...
    ![](/uploads/algo/data-structures/segment-tree-basic/segment-tree-basic_img2.png)
...
![](/uploads/algo/data-structures/segment-tree-basic/segment-tree-basic_img3.png)
...
- Nếu một nút quản lý dãy các đối tượng từ $l$ tới $r$ $(l<r)$ thì nút con trái của nó quản lý các đối tượng từ $l$ tới $\texttt{mid}$ và nút con phải của nó quản lý các đối tượng từ $\texttt{mid}+1$ tới $r$ $\big($Với $\texttt{mid} = \big\lfloor \frac{l + r}{2} \big\rfloor \big)$.
...
2. Phép toán hợp nhất hai nút con trong một cây phân đoạn. **Ví dụ:** Trong cây phân đoạn lấy giá trị nhỏ nhất, hai nút con tương ứng với đoạn $[l_1, r_1]$ và đoạn $[l_2, r_2]$ sẽ được hợp nhất thành một nút tương ứng với đoạn $[l_1, r_2]$ bằng cách lấy *min* các giá trị của hai nút con.
...
![](/uploads/algo/data-structures/segment-tree-basic/segment-tree-basic_gif1.gif)
...
![](/uploads/algo/data-structures/segment-tree-basic/segment-tree-basic_img4.png)
...
- **Ví dụ minh họa:** Cho mảng $A = \{9,2,6,3,1,5,7\}$, ta có $st[] = \{1, 2, 1, 2, 3, 1, 7, 9, 2, 6, 3, 1, 5\}$ (với $st[]$ là mảng biểu diễn cho **Segment Tree**, lưu lại thông tin của mỗi nút).
    ...
    ![](/uploads/algo/data-structures/segment-tree-basic/segment-tree-basic_img5.png)
...
- Cho dãy $A$ gồm $n$ phần tử $a_1, a_2, \ldots, a_n$ $(1 \le n \le 10^{5}; \ \|a_i\| \le 10^{9} \text{ với } 1 \le i \le n)$.
...
int main() {
    ...
    while (q--) {
        ...
        cin >> type >> x >> y;
        if (type == 1)
            update(1, 1, n, x, y); // Gán giá trị y cho phần tử ở vị trí x
        else
...
- Vì vậy, giả sử rằng ta truy cập $3$ hoặc $4$ nút trong độ sâu hiện tại. Từ các nút đó, ta sẽ phân tích kỹ hơn các nút nằm ở giữa (nghĩa là nút thứ hai từ trái sang với số lượng nút đang được truy cập là $3$ và nút thứ $2, \ 3$ với số nút đang được truy cập là $4$) . Vì truy vấn yêu cầu lấy giá trị của một đoạn con liên tục, ta biết rằng các phân đoạn tương ứng với các nút đã thăm ở giữa sẽ được bao phủ hoàn toàn bởi phân đoạn của truy vấn. Do đó các nút này sẽ không thực hiện bất kỳ lệnh gọi đệ quy nào tới các nút con nữa. Vì vậy, chỉ nút bên trái nhất và nút bên phải nhất mới có khả năng tiếp tục gọi đệ quy. Và hai nút đó sẽ chỉ tạo ra nhiều nhất $4$ lệnh gọi đệ quy, vì vậy độ sâu tiếp theo cũng sẽ đáp ứng đúng mệnh đề.
    ...
    ![](/uploads/algo/data-structures/segment-tree-basic/segment-tree-basic_img6.png)
...
$\circ \ 1 \le n, q \le 10^{5}$ (với $q$ là số lượng truy vấn).
...
Cho dãy số $a_1, a_2, \ldots, a_n$ $(\|a_i\| \le 15000, \ n \le 50000)$.
...
- `pre` : Dễ thấy rằng tiền tố lớn nhất của nút cha sẽ bằng *max* tiền tố của nút con bên trái và tổng giá trị của nút con bên trái cộng với tiền tố của nút con bên phải.
    - **Minh họa:** $\texttt{pre}[l, r] = \max(\texttt{pre}[l, \texttt{mid}], \ \texttt{sum}[l, \texttt{mid}] + \texttt{pre}[\texttt{mid} + 1, r])$ với $\texttt{mid} = \big\lfloor \frac{l + r}{2} \big\rfloor$.

    ![](/uploads/algo/data-structures/segment-tree-basic/segment-tree-basic_img7.png)
...
- `maxsum` :
    ...
    - Ở trường hợp thứ $3$, giá trị của đoạn con lớn nhất sẽ là tổng hậu tố lớn nhất của nút con bên trái với tiền tố lớn nhất của nút con bên phải.
        - **Minh họa:** Trường hợp $3$, với $\texttt{mid} = \big\lfloor \frac{l + r}{2} \big\rfloor$ :

        ![](/uploads/algo/data-structures/segment-tree-basic/segment-tree-basic_img8.png)
...
Với mỗi truy vấn, đáp án chính là $\texttt{maxsum}[x, y]$.
...
- Truy vấn loại $2$ có dạng $2 \ L \ R \ k$, ta cần in ra số nhỏ nhất mà lớn hơn bằng $k$ trong khoảng $[L\ldots R]$.

$\circ \ 1 \le N, M \le 10^{5}$ (với $M$ là số lượng truy vấn).
...
- **Ví dụ:** Xét trường hợp tổng quát, với $N = 8$ là lũy thừa của $2$, ta có cây phân đoạn đầy đủ sau:
    ...
    ![](/uploads/algo/data-structures/segment-tree-basic/segment-tree-basic_img9.png)
...
- **Ví dụ:**  Cho mảng $A = \{9,2,6,3,1,5,7\}$ và ta cần cập nhật đoạn $[1, 6]$ :
    ...
    ![](/uploads/algo/data-structures/segment-tree-basic/segment-tree-basic_img10.png)
...
Bạn được cho một mảng gồm $n$ số nguyên $a_1, a_2, \ldots, a_n$ $(|a_i| \le 10^{9},$ với $1 \le i \le n)$. Nhiệm vụ của bạn là xử lí $2$ loại truy vấn:
- Loại $1$ có dạng $1 \ x \ y \ \texttt{val}$ : Tăng giá trị của các phần tử từ vị trí thứ $x$ đến vị trí thứ $y$ trong dãy lên $\texttt{val}$ đơn vị $(1 \le x \le y \le n, 1 \le \texttt{val} \le 10^{9})$.
...
Với truy vấn loại $1$, thao tác cập nhật đoạn $[u, v]$. Giả sử ta gọi $f[id]$ là giá trị lớn nhất trong đoạn mà nút $id$ quản lý. Trong lúc cập nhật, muốn hàm này thực hiện với độ phức tạp không quá $\mathcal{O}(\log{N})$, thì khi xét đến $1$ nút $id$ quản lý đoạn $[l, r]$ mà đoạn $[l, r]$ nằm hoàn toàn trong đoạn $[u, v]$, thì ta không được đệ quy vào các nút con của nó nữa (nếu không độ phức tạp sẽ là $\mathcal{O}(N)$ do ta đi vào tất cả các nút nằm trong đoạn $[u, v]$).

Để giải quyết, ta dùng kĩ thuật **Lazy Propagation** như sau:
- Mỗi nút của **Segment Tree** sẽ lưu thêm một giá trị $\texttt{lazy}[id]$ với ý nghĩa là tất cả các phần tử trong đoạn $[l, r]$ mà nút $id$ quản lý đều được tăng thêm $\texttt{lazy}[id]$. Nói cách khác, nó sẽ lưu trữ các giá trị cần cập nhật của mỗi nút.
- Ban đầu, ta khởi tạo tất cả các phần tử của mảng $\texttt{lazy}[]$ bằng $0$. Nếu $\texttt{lazy}[id] = 0$ có nghĩa là không có cập nhật nào đang chờ được xử lý trên nút $id$ trong cây phân đoạn. Ngược lại, nếu $\texttt{lazy}[id] \neq 0$ nghĩa là ta cần cập nhật giá trị của nút $id$ theo $\texttt{lazy}[id]$ trước khi thực hiện bất kỳ thao tác nào với nút đó *(việc cập nhật này tùy vào mục đích của cây; ví dụ như trong bài toán trên, giá trị của nút $id$ sẽ được tăng thêm $\texttt{lazy}[id]$ )*.
- Nếu nút $id$ đang xét còn có giá trị được thêm vào chưa xét đến (từ các truy vấn cập nhật xảy ra trước đó, mà chưa được cập nhật hết), ta buộc phải cập nhật nó trước khi lấy giá trị hoặc tiếp tục đệ quy sâu hơn. Sau khi cập nhật xong nút $id$, ta phải đẩy giá trị $\texttt{lazy}$ ở nút $id$ xuống các con của nó để tiếp tục cập nhật và đặt lại $\texttt{lazy}$ của nút $id$ về $0$.
- Để làm được điều này, ở đầu các hàm `get` và `update` ta thực hiện các thao tác sau:
    - `st[id] += lazy[id]` - cập nhật giá trị nút $id$ theo $\texttt{lazy}[id]$.
    - Nếu nút $id$ không phải là nút lá thì ta đẩy giá trị $\texttt{lazy}$ xuống các con của nó :
        - `lazy[2 * id] += lazy[id]`
        - `lazy[2 * id + 1] += lazy[id]`
...
- Bây giờ, để cập nhật đoạn, ta thực hiện tương tự như ở **[thao tác lấy giá trị](/algo/data-structures/segment-tree-basic)**, là xác định tập hợp ít nút nhất sao cho tổng tất cả các phạm vi mà các nút đó quản lí đúng bằng đoạn cần cập nhật. Khi đó, ta sửa lại giá trị của các nút thuộc tập hợp này và lưu lại giá trị cần cập nhật vào $\texttt{lazy}$ tương ứng với các nút con của chúng.
...
- $1 \le n \le 10^{5}; \ 0 \le q \le 50000$.
- $1 \le i \le j \le n; \ k \in \{ 0, 1 \}$.
...
- **Ví dụ:** Cho chuỗi $S = "dabedaba"$, giả sử ta cần phải sắp xếp các kí tự của chuỗi $S$ theo thứ tự **tăng dần**:
    - Gọi $\texttt{cnt}[i]$ là số lần xuất hiện của kí tự $i$ trong chuỗi $S$. Ta có thể dễ dàng tính toán mảng $\texttt{cnt}$ bằng cách lấy giá trị từ các cây phân đoạn tương ứng với mỗi kí tự.
    ...
    - Tiếp theo, ta dùng cây phân đoạn để cập nhật mảng tương ứng với kí tự $'a'$ từ vị trí $1$ đến vị trí $3$ (do $\texttt{cnt}[\text{'a'}] = 3$) bằng giá trị $1$ . Rồi lại tiếp tục cập nhật mảng tương ứng với kí tự $'b'$ từ vị trí $4$ đến vị trí $5$ (do $\texttt{cnt}[\text{'b'}] = 2$) bằng giá trị $1$. Vì $\texttt{cnt}[\text{'c'}] = 0$ nên ta bỏ qua kí tự $'c'$. Và lại tiếp tục với kí tự $'d'$, ... đến kí tự $'z'$.
    - Tương tự, để sắp xếp **giảm dần**, ta chỉ cần xét từ kí tự $'z'$ về kí tự $'a'$.

Ta sử dụng kỹ thuật **Lazy Propagation** để cập nhật các đoạn. Khác với ví dụ trên *(ở mục **Bài toán** trong phần **Cập nhật lười (Lazy Propagation)**)*, khi cập nhật nút $id$ quản lí đoạn $[l, r]$ theo giá trị $\texttt{lazy}[id]$ thì giá trị của nút $id$ sẽ được tăng thêm $\texttt{lazy}[id] \times (r - l + 1)$. Vì đây là cây phân đoạn lấy tổng, nên khi mỗi phần tử tăng thêm $\texttt{lazy}[id]$ thì tổng giá trị của cả đoạn $[l, r]$ sẽ tăng thêm $\texttt{lazy}[id] \times (r - l + 1)$.

Vì thao tác cập nhật cây phân đoạn này là gán giá trị ($0$ hoặc $1$) nên ta sẽ khởi tạo giá trị ban đầu của mảng $\texttt{lazy}$ bằng $-1$.
...
- Truy vấn loại $1$ có dạng $1 \ L \ R \ A \ B$, ta cộng thêm vào phần tử thứ $i$ thêm $(i - L)A + B$ đơn vị với mọi $L \le i \le R$.
- Truy vấn loại $2$ có dạng $2 \ L \ R$, ta cần in ra tổng của các phần tử trong khoảng $[L\ldots R]$, lấy dư cho $10^{9} + 7$.

$\circ \ 1 \le N, M \le 10^{5}$ (với $M$ là số lượng truy vấn). Các số trong *input* đều lớn hơn hoặc bằng $1$ và nhỏ hơn hoặc bằng $10^{9}$.

## Phân tích

- Với truy vấn loại $1$: Ta để ý đến giá trị được cộng vào. Với mọi $L \le i \le R$, ta cộng thêm vào phần tử thứ $i$ thêm $(i - L)A + B = iA + B - LA$ đơn vị. Trong đó $B - LA$ là không đổi với mọi phần tử trong truy vấn đề cập đến nên ta có thể dễ dàng sử dụng kĩ thuật **Lazy Propagation** để cập nhật. Còn lại là $iA$ đơn vị, giá trị này thay đổi nên phải dùng mảng $\texttt{lazy}$ thứ $2$ để lưu lại những giá trị dạng này. Nhận thấy rằng, khi mỗi phần tử trong đoạn $[l, r]$ tăng thêm $iA$ đơn vị thì tổng giá trị của cả đoạn $[l, r]$ sẽ tăng thêm $\frac{A \times (r + l) \times (r - l + 1)}{2}$.
    Từ đó, bài toán chuyển về **Segment Tree** bình thường với $2$ mảng $\texttt{lazy}$.
