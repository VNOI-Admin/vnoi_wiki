...
Cho một đồ thị $N \le 2 \times 10^{5}$ đỉnh, và hiện tại thì chưa có cạnh nào. Có $Q (Q \le 10^{5})$ truy vấn, mỗi truy vấn có dạng $\texttt{type}\ u\ v$ biểu thị rằng
- Nếu $\texttt{type} = 1$, thêm cạnh $(u, v)$ vào đồ thị
- Nếu $\texttt{type} = 2$, xoá cạnh $(u, v)$ ra khỏi đồ thị

Sau mỗi truy vấn, hãy in ra số thành phần liên thông trong đồ thị hiện tại.

### Lời giải

Nếu làm như bình thường (thêm/xóa cạnh rồi sau mỗi truy vấn lại BFS/DFS), thì đpt là $\mathcal{O}((N + Q) \times Q)$. 
...
- Buớc 2: Xây dựng một Segment Tree theo thời gian: Nút gốc sẽ quản lý các thời điểm từ $1$ tới $Q$, nút $\texttt{id}$ (quản lý các thời điểm từ $l$ đến $r$) sẽ có $2$ nút con là nút ($\texttt{id} \times 2$) (quản lý các thời điểm từ $l$ đến $\texttt{mid}$) và nút ($\texttt{id} \times 2 + 1$) (quản lý các thời điểm từ $\texttt{mid} + 1$ đến $r$) với $\texttt{mid} = \left\lfloor\frac{l + r}{2}\right\rfloor$
...
- Buớc 4: DFS xuống tất cả các nút của cây. Ở mỗi nút $\texttt{id}$, thêm vào những cạnh mà nút $\texttt{id}$ chứa. Khi xử lý xong nút $\texttt{id}$ và các con của nút này, xóa đi những cạnh đã thêm vào (hay còn gọi là thao tác rollback). Khi đó, mỗi khi thăm một nút lá quản lý thời điểm $[i, i]$, ta có được đáp án cho thời điểm $i$.
...
int rt(int x) { // ta không dùng path compression và chấp nhận đpt O(log2(n)) cho mỗi lần tìm root
...
void merge(int id, int x, int y) { // chúng ta merge như bình thường
    x = rt(x);
    y = rt(y);
    if (x == y)
        return;
...
- Về mặt thời gian: 
    - Với mỗi truy vấn, ta mất $\mathcal{O}(\log{n})$ để cập nhật segment tree (việc cập nhật một nút mất $\mathcal{O}(1)$).  
    - Tuy nhiên, khi nhìn lại vào code mẫu trên, ta thấy hàm $\texttt{rt}()$ không hề sử đặt $\texttt{root}[x] = \texttt{rt}(\texttt{root}[x])$ để giảm độ phức tạp thuật toán xuống như DSU thông thường. Lý do ta không làm như vậy là do ta sẽ gặp rất nhiều khó khăn trong việc thay đổi trở lại $\texttt{root}[x]$, thay vì chỉ cần thay đổi $\texttt{root}[]$ của $2$ đỉnh trong cạnh bị xoá. Vì vậy, độ phức tạp của mỗi lần DSU (cũng như mỗi lần thêm cạnh khi DFS xuống) trong thuật toán này là $\mathcal{O}(\log{n})$, và do đó tổng độ phức tạp của thuật toán là $\mathcal{O}((n + q) \times \log^{2}{(n + q)})$

- Về mặt không gian: $\mathcal{O}(\max(N, Q) \times \log(N + Q))$, đây chính là tổng số cạnh chúng ta thêm vào các vector 
