---
title: Cây chỉ số nhị phân (Binary Indexed Tree)
description: 
published: true
date: 2025-06-26T05:28:54.895Z
tags: 
editor: markdown
dateCreated: 2023-12-25T11:01:07.165Z
---
...
Với truy vấn loại 2, ta duyệt qua tất cả phần tử trong đoạn $[1 \ldots p]$ và cộng giá trị vào biến kết quả.
...
Nếu bài toán không có truy vấn cập nhật, độ phức tạp là $\mathcal{O}(Q + N)$, đủ nhanh để giải quyết. Tuy nhiên, khi có thao tác cập nhật, độ phức tạp bị đẩy lên $\mathcal{O}(Q \cdot N)$ - tương đương với độ phức tạp của cách ngây thơ 1.
...
![](/uploads/algo/data-structures/fenwick/hllVMYY.png)

**Nhận xét:** Mỗi phần tử $\texttt{sum}[i]$ chứa tổng của tất cả phần tử từ $[1\ldots i]$; vì thế, phần tử $\texttt{sum}[i]$ sẽ chứa phần tử $a[j]$ nếu thỏa $i \ge j$, số phần tử $\texttt{sum}$ cần cập nhật là $j - i + 1$, gần tương đương độ dài của mảng.

Để tăng tốc độ cập nhật phần tử, cần bố trí lại phạm vi của từng đoạn gắn với $\texttt{sum}[i]$ để cực tiểu số phần tử $\texttt{sum}$ cần cập nhật nhưng vẫn phải đảm bảo tính liên tục để áp dụng tính chất của prefix sum.
...
![](/uploads/algo/data-structures/fenwick/brL1XCD.png)
...
![](/uploads/algo/data-structures/fenwick/SH4tJPN.png)
...
Để tìm tổng các phần tử trong đoạn $[1 \ldots n]$, ta sẽ lần lượt đi qua tất cả bit của $n$ theo giá trị tăng dần. Mỗi lần đi qua $n$, ta sẽ cộng $\texttt{bit}[n]$ vào kết quả hiện tại, rồi trừ đi bit nhỏ nhất của $n$ khỏi chính nó; quá trình lặp lại cho đến khi $n = 0$.

Để lấy bit nhỏ nhất của một số $n$, ta có thể sử dụng công thức `n & ~(n - 1)` được đề cập tại bài tại [đây](/translate/topcoder/fun-with-bits.md#t%C3%A1ch-t%E1%BB%ABng-bit/). Khi thao tác bit với số âm, C++ sử dụng phép bù 2: `~n = - n - 1`; vì vậy ta có phép biến đổi: `n & ~(n - 1) = n & (-(n - 1) - 1) = n & (-n)` dễ sử dụng hơn.
...
Chứng minh tính đúng đắn của thuật trên như sau: mỗi khi ta cộng thêm 1 lượng bằng với $2^k$ ($k$ là bit nhỏ nhất của $u$) thì đoạn dịch qua phải một lượng $2^k$ thành $[l + 2^k, r + 2^k]$ (do bit nhỏ nhất lúc này vẫn có thể tính là $2^k$). Đồng thời lúc đó, bit nhỏ nhất tăng ít nhất 2 lần do $2^k$ (mới cộng thêm) + $2^k$ (có sẵn trong u) tạo thành $2^{k+1}$ làm cho biên trái dịch trái ít nhất $2^k$ lần thành $[l, r + 2^k]$ (nếu có sẵn $2^{k+1}$ trong $u$ thì tiếp tục gộp lại làm bit nhỏ nhất tăng lên là $2^{k+2}$, ...), do đó biên trái luôn được giữ $\le$ biên $l$ ban đầu.
...
Quay lại bài toán đầu, nếu chúng ta thay đổi yêu cầu thành tìm tổng trên đoạn $[l \ldots r]$, tính chất của prefix sum dễ dàng cho ta tìm được kết quả thông qua phép $\texttt{sum}(r) - \texttt{sum}(l - 1)$. Tuy nhiên, không phải tất cả phép toán nào đều cho phép chúng ta dễ dàng lấy kết quả thông qua phép hiệu như vậy. Đối với các phép $\min, \gcd$, không tồn tại phép hiệu cho ta phép lấy kết quả của một đoạn dễ dàng, vì thế ta không thể áp dụng BIT đối với những bài toán loại này.
...
Ta có thể cài đặt "ngây thơ" bằng cách áp dụng hàm `update()` trên tất cả phần tử cần được cập nhật, độ phức tạp khi này là $\mathcal{O}(Q \cdot N \log N)$. Dĩ nhiên cách này còn chậm hơn việc không sử dụng Fenwick Tree, đòi hỏi ta cần tìm một cách cập nhật đoạn một cách nhanh hơn để giữ được độ phức tạp $\mathcal{O}(Q \log N)$.
...
- Với $i = 1$ thì $\texttt{diff}[i] = A[i]$.
- Với $2 \le i \le N$ thì $\texttt{diff}[i] = A[i] - A[i - 1]$.

Bạn có thể theo dõi hình dưới và code minh họa để hiểu rõ hơn:
![](/uploads/algo/data-structures/fenwick/UaDoTx9.png)
...
Khi cộng tất cả phần tử $\texttt{diff}$ từ $1$ đến $i$, ta có:
$$
\begin{align}
\displaystyle \sum_{j = 1}^{i} \texttt{diff}[j] &= \texttt{diff}[1] + \texttt{diff}[2] + \ldots + \texttt{diff}[i] \\
...
Từ tính chất này, khi tính được mảng hiệu, để tính được giá trị của $a[i]$ ta chỉ cần lấy tổng của $i$ phần tử $\texttt{diff}$ đầu tiên. Khi này, bài toán của chúng ta thực chất được đưa về tính tổng trên mảng $\texttt{diff}$, vấn đề hiện tại là thao tác `update()` cần được xử lí như thế nào.

Hình dưới đây minh họa thao tác cập nhật trên một đoạn $[l \ldots r]$ - từ mảng trên, ta cộng $\Delta = 4$ vào đoạn $[4\ldots 7]$:
![](/uploads/algo/data-structures/fenwick/8CQUuuv.png)

Khi cập nhật, do các phần tử liền kề trong đoạn $[l\ldots r]$ đều được cộng cùng một giá trị $\Delta$ nên hiệu giữa chúng thực chất vẫn không đổi. Khác biệt duy nhất khi cập nhật nằm ở 2 biên của đoạn: giữa $(a_{l - 1}, a_l)$ và $(a_r, a_{r+1})$; vì thế ta chỉ cần cập nhật điểm tại 2 biên trên mảng hiệu và dùng truy vấn lấy tổng để tính giá trị hiện tại của $a_i$.
...
```
## Truy vấn trên đoạn
![](/uploads/algo/data-structures/fenwick/K4d4qmh.png)

Hình trên sẽ giúp ta minh họa trực quan hơn mối quan hệ về tổng các phần tử với mảng $A$ và mảng hiệu $\texttt{diff}$.

Nhắc lại: $A[i] = \sum_{j=1}^{i} \texttt{diff}[i]$. Dựa vào hình, ta có thể tính lần lượt tổng các phần tử từ $A_1$ đến $A_i$ như sau:
- $\texttt{sum}[1] = \texttt{diff}[1]$
- $\texttt{sum}[2] = 2 \cdot \texttt{diff}[1] + \texttt{diff}[2]$
- $\texttt{sum}[3] = 3 \cdot \texttt{diff}[1] + 2 \cdot \texttt{diff}[2] + \texttt{diff}[3]$
- $\ldots$
- $\texttt{sum}[i] = i \cdot \texttt{diff}[1] + (i - 1) \cdot \texttt{diff}[2] + \ldots + (i - j + 1) \cdot \texttt{diff}[j] + \ldots + 2 \cdot \texttt{diff}[i - 1] + \texttt{diff}[i]$

Tuy nhiên, do sự biến động của hệ số khi nhân nên cách này không thuận tiện khi ta phải truy vấn liên tục. Để dễ dàng hơn, ta sẽ cố định mỗi $\texttt{diff}[i]$ nhân với hệ số $n - i + 1$, khi này:
- $\texttt{sum}[1] = n \cdot \texttt{diff}[1] - (n - 1) \cdot \texttt{diff}[1]$
- $\texttt{sum}[2] = n \cdot \texttt{diff}[1] + (n - 1) \cdot \texttt{diff}[2] - (n - 2) \cdot (\texttt{diff}[1] + \texttt{diff}[2])$
- $\texttt{sum}[3] = n \cdot \texttt{diff}[1] + (n - 1) \cdot \texttt{diff}[2] + (n - 2) \cdot \texttt{diff}[3] - (n - 3) \cdot (\texttt{diff}[1] + \texttt{diff}[2] + \texttt{diff}[3])$
- $\ldots$
- $\texttt{sum}[i] = n \cdot \texttt{diff}[1] + (n - 1) \cdot \texttt{diff}[2] + \ldots + (n - j + 1) \cdot \texttt{diff}[j] + \ldots + (n - i + 1) \cdot \texttt{diff}[i] - (n - i) \cdot (\texttt{diff}[1] + \texttt{diff}[2] + \ldots + \texttt{diff}[i])$

Tóm lại, ta thu được:
$$
\texttt{sum}[i] = \displaystyle \sum_{j = 1}^{i} (n - j + 1) \cdot \texttt{diff}[j] - (n - i) \cdot \sum_{j = 1}^{i} \texttt{diff}[j]
$$
Cả hai giá trị $\texttt{diff}[j]$ và $(n - j + 1) \cdot \texttt{diff}[j]$ đã được đơn giản hóa, lúc này ta chỉ cần lưu toàn bộ giá trị $(n - j + 1) \cdot \texttt{diff}[j]$ vào mảng $S_1$ và $\texttt{diff}[j]$ vào mảng $S_2$ và dựng mảng cộng dồn trên hai mảng đó.
...
VNOI Online Judge có phân loại riêng các bài tập về BIT, các bạn có thể tham khảo tại [đây](https://oj.vnoi.info/problems/?type=68&point_start=&point_end=).
