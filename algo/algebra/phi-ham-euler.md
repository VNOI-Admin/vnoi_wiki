...
**Hàm số Euler** hay **phi hàm Euler** của một số nguyên dương $n$ là số lượng số bé hơn hoặc bằng $n$ và nguyên tố cùng nhau với $n$. Kí hiệu của hàm số Euler là $\varphi(n)$.

## Cách tính

Từ định nghĩa, ta có thể tính được $\varphi(p) = p - 1$ và $\varphi(p^k) = p^k - p^{k-1}$ với $p$ là một số nguyên tố.

:::spoiler *Chứng minh*
Đối với $\varphi(p) = p - 1$, ta dễ dàng thấy được rằng với các số $x$ từ $1$ đến $p-1$: $\mathrm{UCLN}(x, p) = 1$ với $\mathrm{UCLN}$ là hàm ước chung lớn nhất của hai số nguyên.

Đối với $\varphi(p^k) = p^k - p^{k-1}$, ta thấy các số $x$ thoả mãn $\mathrm{UCLN}(x, p^k) > 1$ chỉ có thể là các bội số của $p$, tức là các số thuộc tập hợp $\{p, 2p, 3p, \cdots, p^{k-1}p\}$. Vì trong $p^k$ số từ $1$ đến $p$ có $p^{k-1}$ số là bội số của $p$ nên ta có $p^k - p^{k-1}$ số nguyên tố cùng nhau với $p^k$.
:::

Nếu như ta đã biết trước giá trị phi hàm Euler của hai số $a$ và $b$, ta có thể tính được giá trị $\varphi(ab)$.
- Nếu $a, b$ nguyên tố cùng nhau:
$$
\varphi(ab) = \varphi(a) \times \varphi(b)
$$
- Nếu $a, b$ không nguyên tố cùng nhau, với $d = \mathrm{UCLN}(a, b)$: 
$$
\varphi(ab) = \varphi(a) \times \varphi(b) \times \frac{d}{\varphi(d)}
$$
...
Độ phức tạp của thuật toán bằng $\mathcal{O}(\sqrt{n})$.
...
Từ đây, ta dễ dàng tính được giá trị [nghịch đảo modulo](https://wiki.vnoi.info/algo/math/modular-inverse#ngh%E1%BB%8Bch-%C4%91%E1%BA%A3o-modulo-1) của $a$ bằng $a^{\varphi(p)-1} \bmod p$.

### Luỹ thừa

Với hai số $a$ và $p$ nguyên tố cùng nhau, nếu $c \equiv d \pmod{\varphi(p)}$, thì:
...
Độ phức tạp thuật toán bằng $\mathcal{O}(b\log{\log{b}} + \sqrt{a})$.
...
$$
\sum_{i-1}^{N-1} \sum_{j = i + 1}^{N} \mathrm{UCLN}(i, j)
$$

Tất nhiên, chương trình trâu $\mathcal{O}(n^2 \log n)$ sẽ không thể giải quyết bài toán này được. Ta cần tìm một tính chất nào đó của công thức trên giúp ta giải quyết bài toán nhanh hơn.

Đầu tiên, ta viết lại công thức như sau:

$$
\sum_{i-1}^{N-1} \sum_{j = i + 1}^{N} \mathrm{UCLN}(i, j) = \sum_{i = 1}^{n} \mathrm{sum}(i)
$$

Với $\mathrm{sum}(i) = \sum_{j = 1}^{i-1} \mathrm{UCLN}(i, j)$

Ta thấy, với $g = \mathrm{UCLN}(i, j)$, ta suy ra được rằng: $\mathrm{UCLN}\left(\frac{i}{g}, \frac{j}{g}\right) = 1$.

Từ đây, hàm $\mathrm{sum}(n)$ có thể được viết lại bằng $\mathrm{sum}(n) = \sum_{d | n} d \times \varphi\left(\frac{n}{d}\right)$ với $d$ là các ước nguyên dương của $n$.
...
\begin{align*}
\sum_{i-1}^{N-1} \sum_{j = i + 1}^{N} \mathrm{UCLN}(i, j) &= \sum_{i = 1}^{n} \mathrm{sum}(i) \\
&= \sum_{i = 1}^{n} \sum_{j|i} j \times \varphi\left(\frac{i}{j}\right)
\end{align*}
$$
...
Độ phức tạp thuật toán sẽ là $\mathcal{O}(N \log \log {N})$ cho việc tính tất cả các trường hợp của $N$, và $\mathcal{O}(1)$ cho mỗi số $N$ trong các truy vấn.
