...
### Cải tiến
So với các phép toán khác, phép lấy phần dư (`%`) chậm hơn một chút dù vẫn có độ phức tạp là $\mathcal{O}(1)$. Chúng ta có thể xây dựng một cách cài đặt khác không sử dụng phép toán này.
...
Thuật toán cải tiến trên sẽ thực hiện chia $\log(a) + \log(b)$ lần trong trường hợp tệ nhất. Do vậy, độ phức tạp của thuật vẫn không đổi và là $\mathcal{O}(\log \min(a, b))$.
...
### Độ phức tạp
Thuật toán Euclid mở rộng thực tế chỉ là thêm một vài bước tính toán vào thuật toán Euclid thường nên độ phức tạp vẫn là $\mathcal{O}(\log\min(a, b))$.
...
$$
\frac{a}{d} \times x \equiv \frac{c}{d} \ \left(\bmod \frac{b}{d}\right)
$$
...
$$
x \equiv \frac{c}{d} \times \left(\frac{a}{d}\right) ^{-1} \ \left(\bmod \frac{b}{d}\right)
$$
...
$$
\begin{cases}
	x = \frac{b}{d} \times k + \frac{c}{d} \times \gamma &\left(k, \gamma \in \mathbb{Z}, \frac{a}{d} \times \gamma \equiv 1 \ \left(\bmod \frac{b}{d}\right)\right) \\
	y = \frac{c - ax}{b}
\end{cases}
$$
</details>
...
$$
\mathrm{C}^k_n = \frac{n!}{k!\times (n - k)!} \equiv n!\times (k!\times (n - k)!)^{-1} \pmod{M}
$$
