...
![](/uploads/algo/graph-theory/topological-sort/topological_sort_img1.png)
...
- Dễ nhận thấy rằng, một đồ thị **tồn tại thứ tự Tô-pô** thì đồ thị đó phải là đồ thị có hướng và không có chu trình *(Directed Acyclic Graph - DAG)*. Vì nếu đồ thị có chứa một chu trình, $v_1 v_2 \ldots v_n v_1$ thì $v_2$ phải đứng sau $v_1$ theo thứ tự Tô-pô, $v_3$ phải đứng sau $v_2$, v.v...; nhưng sau đó $v_1$ phải đứng sau $v_1$, dẫn đến kết luận vô lý khi $v_1$ phải đứng sau chính nó trong danh sách.
- Vậy câu hỏi đặt ra là liệu điều kiện này có đủ hay không? Có đúng là một đồ thị có thể được sắp xếp theo thứ tự Tô-pô khi và chỉ khi đồ thị đó là một $\texttt{DAG}$ ? Câu trả lời là ***đúng***.
...
- **Chứng minh :**
    * Rõ ràng, một $\texttt{DAG}$ chỉ gồm $1$ đỉnh duy nhất luôn có thể được sắp xếp theo thứ tự Tô-pô. Khi đó, danh sách Tô-pô trong trường hợp này chỉ bao gồm chính đỉnh đó.
    * Giả sử rằng bất kỳ $\texttt{DAG}$ nào có $n$ đỉnh đều có thể được sắp xếp theo thứ tự Tô-pô. Bây giờ hãy xem xét các $\texttt{DAG}$ có $n+1$ đỉnh. Nhớ lại rằng mọi $\texttt{DAG}$ đều có ít nhất một **đỉnh nguồn** (đỉnh không có cung vào). Hãy để **đỉnh nguồn** này làm giá trị đầu tiên trong danh sách Tô-pô, rồi loại bỏ đỉnh này cùng cách cạnh kề với nó ra khỏi đồ thị. Khi đó, ta sẽ có được đồ thị mới gồm $n$ đỉnh là đồ thị con của đồ thị ban đầu. Rõ ràng, các chu trình không thể được tạo ra bằng cách loại bỏ các cạnh và đỉnh. Bằng giả thiết ban đầu, ta có thể sắp xếp đồ thị con này theo thứ tự Tô-pô. Sau đó, ta nối thứ tự Tô-pô của đồ thị con vào cuối danh sách hiện tại để có được danh sách thứ tự Tô-pô của đồ thị $n+1$ đỉnh với **đỉnh nguồn** là giá trị đầu tiên trong danh sách.
    * **Ví dụ minh họa:** Để xác định thứ tự Tô-pô của $\texttt{DAG}$ gồm $7$ đỉnh, ta thêm đỉnh nguồn $0$ vào trong danh sách Tô-pô rồi nối thêm thứ tự Tô-pô của đồ thị con $6$ đỉnh.
    ![](/uploads/algo/graph-theory/topological-sort/topological_sort_img2.png)
    ...
    * Do đó, danh sách $n+1$ đỉnh mới này là một thứ tự Tô-pô của $\texttt{DAG}$ gồm $n+1$ đỉnh.
- **Tính chất**
    * **Thứ tự Tô-pô không nhất thiết phải là duy nhất**. Có thể có một số thứ tự Tô-pô khác nhau trong một đồ thị.
    * Tuy nhiên, thứ tự Tô-pô sẽ là duy nhất khi $\texttt{DAG}$ có đường đi $\text{Hamilton}$.
...
**Note**
![](/uploads/algo/graph-theory/topological-sort/topological_sort_img3.png)
...
- Từ phần **chứng minh** của định lý trên *(trong mục **Điều kiện tồn tại thứ tự Tô-pô**)*, ta có thuật toán để tìm ra một thứ tự Tô-pô như sau:
    ...
    2. Tìm đỉnh nguồn của $\texttt{DAG}$. Thêm đỉnh này vào cuối danh sách Tô-pô.
    3. Loại bỏ đỉnh này và tất cả các cạnh kề với nó ra khỏi $\texttt{DAG}$.
    4. Nếu số đỉnh của $\texttt{DAG}$ lớn hơn $0$, hãy quay lại bước $2$.
- Sau khi kết thúc, danh sách sẽ chứa một thứ tự Tô-pô của $\texttt{DAG}$.
...
- Thay vào đó, ta có những **nhận xét** sau:
	...
	* Giả sử ta biết vị trí của tất cả các đỉnh nguồn trong $\texttt{DAG}$ và ta thực hiện các bước $2$ và $3$ một lần. Sau đó, những đỉnh duy nhất có thể **trở thành đỉnh nguồn** là những đỉnh có cung vào nối với đỉnh vừa bị xóa.
...
![](/uploads/algo/graph-theory/topological-sort/topological_sort_gif1.gif)
...
- Thuật toán sẽ luôn tìm ra một thứ tự Tô-pô nếu nó tồn tại. Còn nếu nó **không** tồn tại (nghĩa là đồ thị ban đầu **không phải** là $\texttt{DAG}$ ) thì danh sách `topo` sẽ không được điền đầy đủ và thông báo lỗi sẽ được in ra.
- Thứ tự Tô-pô sẽ là duy nhất khi và chỉ khi `listSource` chứa chính xác một đỉnh ở đầu mỗi lần lặp của vòng lặp *while*.

**Độ phức tạp**
- Ở vòng lặp *while* để duyệt các đỉnh trong `listSource`, ta mất độ phức tạp $\mathcal{O}(n)$ vì mỗi đỉnh chỉ được thêm vào nhiều nhất $1$ lần *(ở ban đầu hoặc ngay sau khi cạnh vào cuối cùng của nó bị loại bỏ)*. Và ta sẽ mất thêm độ phức tạp $\mathcal{O}(m)$ để duyệt các cạnh vì mỗi cạnh chỉ được kiểm tra nhiều nhất $1$ lần trong vòng lặp while *(khi đỉnh nối với nó được lấy ra)*.
- Nhìn chung, độ phức tạp của thuật toán này là $\mathcal{O}(n+m)$.
...
- **Ví dụ minh họa:**
![](/uploads/algo/graph-theory/topological-sort/topological_sort_img4.png)
- **Chứng minh:** Hãy xem xét **cây *DFS*** của đồ thị *(tìm hiểu thêm về cây $\texttt{DFS}$ tại [đây](/algo/graph-theory/Depth-First-Search-Tree.md))*:
    * Nếu $u$ là tổ tiên của $v$ trong cây $\texttt{DFS}$, thì quá trình duyệt xong cây con gốc $v$ sẽ kết thúc sớm hơn quá trình duyệt xong cây con gốc $u$. Do đó, $u$ sẽ có số thứ tự cao hơn $v$ trong danh sách.
    * Nếu $u$ là tổ tiên của $v$ trong cây $\texttt{DFS}$ và tồn tại cung nối từ $1$ đỉnh thuộc cây con gốc $v$ đến đỉnh $u$ thì đồ thị có chu trình (không phải là *DAG*) nên sẽ không tồn tại thứ tự Tô-pô.
    * Nếu $u$ **không phải** là tổ tiên của $v$ và $v$ cũng **không phải** là tổ tiên của $u$ trong cây $\texttt{DFS}$ thì khi đó, $2$ đỉnh $u$ và $v$ sẽ nằm trên $2$ nhánh khác nhau của cây $\texttt{DFS}$. Do đó cung nối từ nhánh chứa đỉnh $u$ đến nhánh chứa đỉnh $v$ là **cung chéo** và đỉnh $u$ sẽ được duyệt sau $v$ nên sẽ có số thứ tự cao hơn $v$ trong danh sách.
        * **Ví dụ minh họa:** Bắt đầu duyệt $\texttt{DFS}$ từ đỉnh $S$. Các "cạnh nét liền" là cung của cây $\texttt{DFS}$. Các "cạnh nét đứt" **không phải** là cung của cây $\texttt{DFS}$. Với cung chéo $u \rightarrow v$, đỉnh $v$ chắc chắn sẽ được duyệt trước $u$. Vì nếu $u$ được duyệt trước $v$ thì đỉnh $u$ sẽ là **tổ tiên** của đỉnh $v$ và cung $u \rightarrow v$ sẽ không còn là cung chéo nữa, khi đó ta xét giống như ở trường hợp đầu tiên.
        ![](/uploads/algo/graph-theory/topological-sort/topological_sort_img5.png)
- Từ những nhận xét trên, ta có thuật toán để tìm ra một thứ tự Tô-pô như sau:
    * Xuất phát từ một điểm chưa được duyệt, ta bắt đầu duyệt $\texttt{DFS}$ trên đồ thị xuất phát từ điểm đó.
    * Dùng một mảng để lưu trạng thái của mỗi đỉnh. Có $3$ trạng thái:
        * **Trạng thái 0** : Đỉnh chưa được duyệt (chưa từng được gọi hàm $\texttt{DFS}$).
        * **Trạng thái 1** : Đỉnh vẫn đang duyệt (hàm $\texttt{DFS}$ với đỉnh này chưa kết thúc).
        * **Trạng thái 2** : Đỉnh đã duyệt xong (hàm $\texttt{DFS}$ với đỉnh này đã kết thúc).
    ...
    * Sau khi kết thúc duyệt $\texttt{DFS}$ trên đồ thị, thứ tự hoàn tất duyệt của mỗi đỉnh chính là **danh sách nghịch đảo thứ tự Tô-pô**.
...
![](/uploads/algo/graph-theory/topological-sort/topological_sort_gif2.gif)
...
- Với phương pháp sắp xếp Tô-pô bằng $\texttt{DFS}$, nếu đồ thị không phải là $\texttt{DAG}$ thì ta vẫn có thể tìm ra một thứ tự, nhưng ta lại có thế dùng $\texttt{DFS}$ để kiểm tra luôn xem đồ thị có là $\texttt{DAG}$ hay không.
- Nếu đồ thị **không phải** là $\texttt{DAG}$ thì thông báo lỗi sẽ được in ra.

**Độ phức tạp**
- Ở hàm $\texttt{DFS}$, ta mất độ phức tạp $\mathcal{O}(n)$ vì mỗi đỉnh chỉ được duyệt nhiều nhất $1$ lần và ta sẽ mất thêm độ phức tạp $\mathcal{O}(m)$ để duyệt tất cả các cạnh của đồ thị.
- Nhìn chung, độ phức tạp của thuật toán này là $\mathcal{O}(n + m)$.
...
Cho đồ thị có hướng gồm $N$ đỉnh và $M$ cạnh. Các đỉnh được đánh số $1,2,\ldots,N$ và với mỗi $i$ $(1 \le i \le M)$, cạnh có hướng thứ $i$ sẽ đi từ đỉnh $x_i$ đến đỉnh $y_i$. $G$ không chứa bất kì chu trình có hướng nào !
...
- Gọi $\texttt{dp}[i]$ là độ dài đường đi dài nhất bắt đầu từ đỉnh có chỉ số $i$. Ta có công thức quy hoạch động trên danh sách Tô-pô như sau:
    * $\texttt{dp}[\texttt{topo}[i]]=\max(\texttt{dp}[\texttt{topo}[j]] + 1)$ (Với $i = n \rightarrow 1$ và tồn tại cung từ $\texttt{topo}[i] \rightarrow \texttt{topo}[j]$).
- Theo định nghĩa của thứ tự Tô-pô, chỉ tồn tại các cung từ chỉ số nhỏ đến chỉ số lớn hơn. Nên khi ta tính toán đến $\texttt{dp}[\texttt{topo}[i]]$ thì chắc chắn $\texttt{dp}[\texttt{topo}[j]]$ đều đã được tính trước đó.
- Kết quả độ dài đường đi dài nhất của đồ thị là $\max(\texttt{dp}[i])$.
...
- Khi sắp xếp Tô-pô bằng $\texttt{DFS}$, thuật toán sẽ trả ra **danh sách nghịch đảo thứ tự Tô-pô** *(định nghĩa đã được viết ở mục **Bài toán 1** phần **Thuật toán 2**)*. Mà ở công thức quy hoạch động, ta cần duyệt ngược lại danh sách, nên ta sẽ giữ nguyên thứ tự **nghịch đảo** đó và xử lí luôn trên nó.
...
- Với **đồ thị tổng quát**, bài này sẽ trở thành bài toán $\texttt{NP}$.
- Với $\texttt{DAG}$, quy hoạch động trên thứ tự Tô-pô **có thể giải được** trong $\mathcal{O}(n + m)$, kể cả với trọng số dương/âm, kể cả tìm đường đi ngắn nhất/dài nhất.
...
**Thứ tự từ điển** được xác định theo cách sau: Khi so sánh $2$ xâu kí tự $s$ và $t$, đầu tiên ta tìm vị trí ngoài cùng bên trái sao cho các ký tự khác nhau $(s_i \ne t_i)$ rồi so sánh các ký tự $s_i$ và $t_i$ theo thứ tự của chúng trong *bảng chữ cái*, nếu $s_i < t_i$ thì $s$ có thứ tự từ điển nhỏ hơn $t$ và ngược lại. Còn nếu không có vị trí như vậy (tức là $s$ là tiền tố của $t$ hoặc ngược lại) thì chuỗi ngắn hơn sẽ có thứ tự từ điển nhỏ hơn.

**Input**

- Dòng đầu tiên chứa số nguyên dương $n$ là số lượng tên các tác giả.
- $n$ dòng tiếp theo, mỗi dòng chứa một chuỗi kí tự $\texttt{name}_i$ $(1 \le \|\texttt{name}_i\| \le 100)$ gồm các chữ cái *Latin* in thường là tên của tác giả thứ $i$. Tất cả tên của các tác giả đều khác nhau.
...
- Nếu không, in ra một từ duy nhất: $\texttt{Impossible}$
...
* Giả sử, ta có hai xâu kí tự $s$ và $t$. Theo **định nghĩa của thứ tự từ điển**, hãy xem xét xem nếu $s < t$ thì nó cho ta biết những dữ kiện gì.
    * Ví dụ: $s="\texttt{abcxyz}"$ và $t="\texttt{abcuv}"$. Để $s < t$ thì $'x'$ phải có thứ tự nhỏ hơn $'u'$ trong *bảng chữ cái*.
...
* Nếu đồ thị **không phải** là $\texttt{DAG}$ thì **không tồn tại** thứ tự chữ cái nào thỏa mãn.
...
**Độ phức tạp**
- Ở vòng *for* đầu tiên để xây dựng các cạnh của đồ thị, ta mất độ phức tạp $\mathcal{O}(n \times \|\texttt{name}\|)$.
- Ở vòng *for* thứ hai để xác định thứ tự Tô-pô, ta mất độ phức tạp $\mathcal{O}(26 + m)$. Với $m$ là số cạnh của đồ thị. Trong **trường hợp xấu nhất**, với mỗi cặp kí tự liên tiếp đều tạo ra $1$ cạnh thì số lượng cạnh lớn nhất có thể tạo ra là $n-1$ cạnh.
- Nhìn chung, độ phức tạp của thuật toán này là $\mathcal{O}(n \times \|\texttt{name}\|)$.
