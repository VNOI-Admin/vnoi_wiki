...
### Mệnh đề
Mệnh đề hay mệnh đề logic được định nghĩa là một câu khẳng định về một vấn đề nào đó. Một mệnh đề có thể là đúng [$\texttt{True}$, $\texttt{1}$] hoặc sai [$\texttt{False}$, $\texttt{0}$], không thể vừa đúng, vừa sai.

Ví dụ: 
+ "$\texttt{1 + 1 = 2}$" là một mệnh đề đúng. "$\texttt{1 + 1 = 2}$" $= \texttt{True}$
+ "$\texttt{1 + 1 = 3}$" là một mệnh đề sai. "$\texttt{1 + 1 = 3}$" $= \texttt{False}$
...
|$\texttt{False}$|$\texttt{True}$|$\texttt{True}$|$\texttt{False}$|$\texttt{True}$|$\texttt{True}$|$\texttt{False}$|
|$\texttt{False}$|$\texttt{False}$|$\texttt{True}$|$\texttt{False}$|$\texttt{False}$|$\texttt{True}$|$\texttt{True}$| -->
|$A$|$B$|$\lnot A$|$\lnot B$|$A \land B$|$A \lor B$|$A \Rightarrow B$|$A \Leftrightarrow B$|
|---|---|---|---|---|---|---|---|
|$\texttt{1}$|$\texttt{1}$|$\texttt{0}$|$\texttt{0}$|$\texttt{1}$|$\texttt{1}$|$\texttt{1}$|$\texttt{1}$|
|$\texttt{1}$|$\texttt{0}$|$\texttt{0}$|$\texttt{1}$|$\texttt{0}$|$\texttt{1}$|$\texttt{0}$|$\texttt{0}$|
|$\texttt{0}$|$\texttt{1}$|$\texttt{1}$|$\texttt{0}$|$\texttt{0}$|$\texttt{1}$|$\texttt{1}$|$\texttt{0}$|
|$\texttt{0}$|$\texttt{0}$|$\texttt{1}$|$\texttt{1}$|$\texttt{0}$|$\texttt{0}$|$\texttt{1}$|$\texttt{1}$|

### Dạng chuẩn hội (CNF)
Dạng chuẩn hội (CNF) hay Conjunctive Normal Form là hội ($\land$) của một hoặc nhiều mệnh đề. Các mệnh đề này có dạng là tuyển ($\lor$) của các giá trị ($\texttt{True} / \texttt{False}$).
Ví dụ: 
...
### Bài toán SAT
SAT hay Boolean satisfiability problem là một bài toán kiểm tra tồn tại cách gán $\texttt{True}/\texttt{False}$ cho một tập giá trị, sao cho thoả mãn CNF cho trước.
Ví dụ: 
+ Cho dạng chuẩn hội $C = (x_1 \lor \lnot x_2) \land (x_1 \lor x_2 \lor x_3)$
+ Với cách gán $x_1 = \texttt{False}$, $x_2 = \texttt{False}$, $x_3 = \texttt{True}$. $C = (\texttt{False} \lor \texttt{True}) \land (\texttt{False} \lor \texttt{False} \lor \texttt{True})$ $= \texttt{True} \land \texttt{True}$ $= \texttt{True}$ $\Rightarrow$ Thoả mãn CNF.
+ Với cách gán $x_1 = \texttt{False}$, $x_2 = \texttt{False}$, $x_3 = \texttt{False}$. $C = (\texttt{False} \lor \texttt{True}) \land (\texttt{False} \lor \texttt{False} \lor \texttt{False})$ $= \texttt{True} \land \texttt{False}$ $= \texttt{False}$ $\Rightarrow$ Không thoả mãn CNF.
...
**1. Ít nhất một giá trị là $\texttt{True}$:** $C = (u \lor v)$
**2. 2 giá trị  trái dấu:** $C = (u \lor v) \land (\lnot u \lor \lnot v)$
**3. 2 giá trị cùng dấu:** $C = (\lnot u \lor v) \land (u \lor \lnot v)$
**4. Bắt buộc một giá trị mang giá trị $\texttt{True}$:** $C = (u \lor u)$
**5. Bắt buộc một giá trị mang giá trị $\texttt{False}$:** $C = (\lnot u \lor \lnot u)$
...
+ Tìm cách gán $\texttt{True/False}$ cho tập $a_1, a_2, \ldots, a_n$ thoả mãn 2-CNF đã cho.
### Quy ước
...
+ $\texttt{scc}(u)$: tập tất cả các đỉnh cùng thuộc một TPLTM chứa $u$.
...
Tìm cách gán $\texttt{True/False}$ cho tập các đỉnh sao cho thoả mãn **$2$ điều kiện**:
+ **Điều kiện 1:** $2$ đỉnh $a_j, \lnot a_j$ phải trái dấu nhau.
+ **Điều kiện 2:** Với $u = \texttt{True}$ và $(u, v) \in E$ thì $v = \texttt{True}$. 
...
+ **Nhận định 1:** Nếu $u = \texttt{True}$ thì $\forall v \in \texttt{child}(u)$ ta có $v = \texttt{True}$.
+ **Nhận định 2:** Nếu $u = \texttt{False}$ thì $\forall v \in \texttt{par}(u)$, ta có $v = \texttt{False}$

Khi $u = \texttt{True}$ thì $\forall v \in \texttt{scc}(u)$ ta có $u \twoheadrightarrow v \Rightarrow v \in \texttt{child}(u) \Rightarrow v = \texttt{True}$. 
Khi $u = \texttt{False}$ thì $\forall v \in \texttt{scc}(u)$ ta có $v \twoheadrightarrow u \Rightarrow v \in \texttt{par}(u) \Rightarrow v = \texttt{False}$. 
Do đó, với các đỉnh cùng thuộc TPLTM phải mang cùng giá trị $\texttt{True/False}$.
</details>
...
Sau khi thực hiện thuật toán Tarjan, ta sẽ xác định được mảng $\texttt{id}[u]$ $-$ Vị trí của $\texttt{scc}(u)$ trong thứ tự **Tô-pô ngược** bất kỳ (Đối với thuật toán Kosaraju, mảng $\texttt{id}[u]$ là vị trí $\texttt{scc}(u)$ trong thứ tự Tô-pô xuôi bất kỳ):
+ Nếu $\texttt{id}[u] = \texttt{id}[v]$ thì $u, v$ cùng thuộc TPLTM. 
+ Nếu $\texttt{id}[u] < \texttt{id}[v]$ thì $\texttt{scc}(u)$ nằm bên trái $\texttt{scc}(v)$ trong thứ tự Tô-pô ngược.
+ Nếu $\texttt{id}[u] > \texttt{id}[v]$ thì $\texttt{scc}(u)$ nằm bên phải $\texttt{scc}(v)$ trong thứ tự Tô-pô ngược.
+ Nếu $u \twoheadrightarrow v$ thì $\texttt{id}[u] \ge \texttt{id}[v]$.
 
Với mỗi giá trị $u \in \{ a_1, a_2, \ldots, a_m \}$, ta xét:
+ Nếu $\texttt{id}[u] = \texttt{id}[\lnot u]$ thì thông báo bài toán vô nghiệm và kết thúc.
+ Nếu $\texttt{id}[u] < \texttt{id}[\lnot u]$ thì gán $u = \texttt{True}$ và $\lnot u = \texttt{False}$.
+ Nếu $\texttt{id}[u] > \texttt{id}[\lnot u]$ thì gán $u = \texttt{False}$ và $\lnot u = \texttt{True}$.
...
+ ***Chứng minh điều kiện 2: Nếu $u = \texttt{True}$ và $(u, v) \in E$ thì $v = \texttt{True}$.***
    Ta sẽ chứng minh bằng phản chứng, giả sử tồn tại $u = \texttt{True}$ và $(u, v) \in E$ mà $v = \texttt{False}$: 
    + Do $u = \texttt{True}$ nên $\texttt{id}[\lnot u] > \texttt{id}[u]$. Do $v = \texttt{False}$ nên $\texttt{id}[v] > \texttt{id}[\lnot v]$. Ngoài ra, vì $u \to v$ nên $\texttt{id}[\lnot u] > \texttt{id}[u] \ge \texttt{id}[v] > \texttt{id}[\lnot v]$. 
    + Mặt khác, theo cách xây dựng đồ thị $G$, ta cũng phải có $\lnot v \to \lnot u \Leftrightarrow \texttt{id}[\lnot v] \ge \texttt{id}[\lnot u]$ và điều này mâu thuẫn điều kiện trên (Giả thiết được chứng minh là sai).
    
    $\Rightarrow$ Nếu $u = \texttt{True}$ và $(u, v) \in E$ thì $v = \texttt{True}$.
...
Độ phức tạp để thực hiện thuật toán Tarjan cũng như cả bài toán là $\mathcal{O}(n + q)$
...
Sử dụng **thuật toán 2-SAT**, ta giải quyết bài toán trong $\mathcal{O}(n + q)$.
