...
struct MinCostFlow {
    ...
    MinCostFlow(int _n, int _s, int _t) : n(_n), s(_s), t(_t), adj(_n), vst(_n), pi(_n, 0), p(_n), d(_n) {
    ...
    void addEdge(int u, int v, long long c, long long w, long long bc = 0) {
        ...
        edge.emplace_back(Edge(u, v, c, w)); // cạnh xuôi
        adj[v].emplace_back(edge.size());
        edge.emplace_back(Edge(v, u, bc, -w)); // cạnh ngược
    ...
    pair<long long, long long> dijkstra() {
        ...
        for (int i = 0; i < n; ++i)
            pi[i] = min(pi[i] + d[i], INF); // cập nhật giá trị

        if (!vst[t])
            return {-1, -1}; // không tìm thấy đường tăng luồng
...
Gọi $F$ là giá trị của luồng cực đại trên mạng. Vì số lượng đường tăng luồng tối ta có thể tìm được là $\mathcal{O}(F)$ nên khi sử dụng thuật toán Johnson để tìm đường tăng luồng, độ phức tạp của thuật toán tìm luồng cực đại với chi phí cực tiểu của ta bằng $\mathcal{O}(nm + Fm\log{n})$.
...
long long sendKFlow(long long k) {
    ...
    if (k != 0)
        return INF; // không thể gửi K luồng trên mạng
    return cost;    // chi phí cực tiểu
}
```

Độ phức tạp khi này bằng $\mathcal{O}(nm + Km\log{n})$.
