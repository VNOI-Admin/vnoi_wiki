...
A(\omega_n^{i+n/2})&=A_0(\omega_{n/2}^{i})-\omega_n^{i}A_1(\omega_{n/2}^{i})
\end{align}$$
...
Định nghĩa một mảng $\texttt{root}$ sao cho với mỗi $len$ và với mỗi $0\le j<\frac{len}{2}$ ta có $\texttt{root}\left(\frac{len}{2}+j\right)=\omega_{len}^{j}$.
...
Ta chứng minh được rằng với một số nguyên tố có dạng $p=c \cdot 2^{k}+1$, tồn tại $\omega_{2^{k}}=g^{c}$ với $g$ là một [căn nguyên thuỷ](https://vi.wikipedia.org/wiki/C%C4%83n_nguy%C3%AAn_th%E1%BB%A7y_modulo_n) modulo $p$. Lấy ví dụ với $p=998244353=119\cdot 2^{23}+1$ có căn nguyên thuỷ $g=3$ và $\omega_{2^{23}}=3^{119} \bmod p=15311432$.
