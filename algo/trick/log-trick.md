...
> Định lý Euler cho chúng ta biết rằng với $\gcd(a, n) = 1$, $a^{x} \bmod n$ tuần hoàn với chu kì là $\varphi(n)$. Đây là công cụ đắc lực để giản lược số mũ trong tháp luỹ thừa.
> 1. Hàm hợp phi $\varphi^{k}(n) = \varphi(\varphi(\dots \varphi(n) \dots))$ sẽ hội tụ về $1$ sau khoảng $\mathcal{O}(\log n)$ bước.
...
> > Ta sẽ xem xét tốc độ giảm giá trị của $n$ qua mỗi lần áp dụng hàm $\varphi$.
> > 1. **Trường hợp 1: $n$ chẵn ($n > 2$):**
> > Theo công thức tính hàm phi Euler: $\varphi(n) = n \prod_{p \mid n} \left(1 - \frac{1}{p}\right)$.Vì $n$ chẵn nên $n$ có ước nguyên tố là 2. Trong tích trên sẽ chứa thừa số $(1 - \frac{1}{2}) = \frac{1}{2}$. Các thừa số còn lại $(1 - \frac{1}{p})$ đều nhỏ hơn hoặc bằng 1. Do đó:
> > $$
> > \varphi(n) \le n \cdot \frac{1}{2} = \frac{n}{2}
> > $$
> > 2. **Trường hợp 2: Nếu $n$ là số lẻ ($n > 1$)**
> > Nếu $n$ là số lẻ, thì mọi ước nguyên tố $p$ của $n$ đều là số lẻ. Khi đó ta có: $\varphi(n) = \prod p_i^{k_i-1}(p_i - 1)$.
> > Vì $p_i$ là lẻ nên $(p_i - 1)$ là số chẵn. $\Rightarrow$ $\varphi(n)$ với $n$ lẻ luôn là số chẵn với mọi $n > 2$.
> > 
> > Kết hợp 2 trường hợp:
> > 1. Nếu $n$ chẵn: $\varphi(n) \le \frac{n}{2}$.
> > 2. Nếu $n$ lẻ: $\varphi(\varphi(n)) \le \frac{\varphi(n)}{2} < \frac{n}{2}$.
> > 
> > Như vậy, tối đa sau 2 lần áp dụng hàm $\varphi$, giá trị của $n$ sẽ giảm đi ít nhất một nửa. Gọi $T(n)$ là số bước để $n$ giảm về 1. Ta có bất đẳng thức truy hồi:
> > $$
> > T(n) \le T\left(\frac{n}{2}\right) + 2
> > $$
> > 
> > Áp dụng Định lý tổng quát (Master theorem), ta tính được $T(n) \approx 2 \log_2 n$. Nên số bước để dãy hội tụ về 1 là $\mathcal{O}(\log n)$.
>
...
> [!example]- Lời giải
> Ta cần tính đáp án theo modulo $m = 10^{10}$. Trước tiên, ta tính chu kì Pisano của $m$ và có $\pi(m) = 1.5 \times 10^{9}$. Bài toán khi đó chuyển về tính $7^{7^{7^{n}}} \bmod \pi(m)$. Ta có thể áp dụng được định lý Euler do $\gcd(7, \varphi(\pi(m))) = \gcd(7, \varphi^{2}(\pi(m))) = \gcd(7, \varphi^{3}(\pi(m))) = 1$ để tính được tháp luỹ thừa này, sau đó nhân ma trận để tính ra đáp án.
> 
...
> 
> 1. **Tuần hoàn thuần túy**: Lặp lại ngay từ đầu (chẳng hạn Fibonacci theo modulo 3: $0, 1, 1, 2, \ldots, 0, 1, \ldots$).
...
> 
> $$
> a^{H} \equiv a^{H \bmod{\varphi(m)}} \equiv H \pmod{m}
> $$
> 
> Thay $H$ trong phương trình trên bởi $x$, ta có hệ phương trình đồng dư:
> 
> $$
> \begin{cases} x \equiv H \pmod{m} \\ x \equiv H \pmod {\varphi(m)} \end{cases}
> $$
> 
