...
Bây giờ, quay lui cho với mỗi tập $X$ và $Y$, ta được $2$ tập $A$ và $B$ chứa các cặp (tổng trọng lượng $\texttt{sumW}$, tổng giá trị $\texttt{sumV}$) của các tập con.

Để kết hợp $2$ tập $A$ và $B$, ta cần giải quyết bài toán con: Với mỗi cặp $(\texttt{sumW}_i, \texttt{sumV}_i)$ của tập $A$, ta cần tìm một cặp $(\texttt{sumW}_j, \texttt{sumV}_j)$ trong tập $B$ sao cho $\texttt{sumW}_j\le M-\texttt{sumW}_i$ và $\texttt{sumV}_j$ là lớn nhất.

Để giải bài toán con này, gợi ý là sắp xếp lại mảng $B$ theo thứ tự tăng dần của $\texttt{sumW}_j$ và đặt $\texttt{maxSumV}_j=\max(\texttt{sumV}_1, \ldots, \texttt{sumV}_j)$ (phần này có thể tính nhanh bằng [mảng cộng dồn](https://vnoi.info/wiki/algo/data-structures/prefix-sum-and-difference-array.md)).
