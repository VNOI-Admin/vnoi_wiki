...
Tính nhanh hàm tiền tố là hết sức quan trọng vì nó là một bước thiết yếu trong thuật toán KMP. Ở phần sau, bài viết giới thiệu một thuật toán "ngây thơ" để tính hàm tiền tố với độ phức tạp $\mathcal{O}(n^{3})$. Từ đó, ta sẽ dùng các nhận xét để tối ưu thuật toán thành độ phức tạp $\mathcal{O}(n)$.
...
Do có $\mathcal{O}(n^{2})$ cặp $(i, k)$ và so sánh hai xâu bằng `s.substr()` mất $\mathcal{O}(n)$, độ phức tạp của thuật toán này là $\mathcal{O}(n^{3})$.
...
Chỉ riêng nhận xét này là đủ để chúng ta giảm độ phức tạp xuống còn $\mathcal{O}(n^{2})$: Thay vì mỗi lần duyệt $i$ xét mọi giá trị có thể của $k$, thuật toán bắt đầu với $k = \pi[i-1] + 1$ (hàm tiền tố tăng tối đa $1$ nên đây là giá trị lớn nhất có thể của $k$) và giảm dần $k$ đến khi so khớp thành công. Vì hàm tiền tố suốt cả quá trình duyệt $i$ chỉ có thể tăng $k$ tối đa $1 \times n = n$ bước, dẫn tới chỉ có tối đa $n$ bước giảm $k$, nên số bước thực hiện so sánh hai xâu chỉ còn là $\mathcal{O}(n)$. Vì vậy, độ phức tạp tổng của thuật chỉ còn là $\mathcal{O}(n^{2})$ (so sánh xâu vẫn là $\mathcal{O}(n)$).
...
## Phép tối ưu thứ hai
Trước tiên, ta nhận thấy nhược điểm làm cho thuật toán có độ phức tạp $\mathcal{O}(n^{2})$ là thao tác so sánh xâu mất $\mathcal{O}(n)$.
...
Từ đó, ta có thể duyệt qua mọi hậu tố kết thúc tại $i$ khớp với tiền tố độ dài $j$ như sau: Ban đầu đặt $j = \pi[i]$, để đến độ dài tiếp theo thỏa mãn, ta gán $j = \pi[j-1]$. Tương tự lập luận ở phần [phép tối ưu đầu tiên](#phép-tối-ưu-đầu-tiên), khi chuyển từ $i$ sang $i+1$ ta gán $j = \pi[i] = j+1$ nên $j$ tăng tối đa $1$, và khi cập nhật $j = \pi[j-1]$ thì $j$ giảm ít nhất $1$. Do đó, ta chỉ duyệt qua $\mathcal{O}(n)$ giá trị của $j$ và mỗi lần thao tác so sánh hai ký tự mất $\mathcal{O}(1)$, đẫn đến độ phức tạp tổng là $\mathcal{O}(n)$.
...
```
Code có độ phức tạp $\mathcal{O}(n)$ và sử dụng $\mathcal{O}(n)$ bộ nhớ.
...
Về mặt bộ nhớ, ta vẫn cần phải lưu lại xâu và giá trị các hàm tiền tố trước đó nên vẫn cần đến $\mathcal{O}(n)$ bộ nhớ. Trường hợp đặc biệt là khi hàm tiền tố luôn nhỏ hơn một hằng số $M$ nào đó, khi đó ta chỉ cần lưu $M + 1$ chữ cái đầu và $M + 1$ hàm tiền tố tương ứng của xâu. Nhận xét này sẽ tỏ ra rất hữu ích ở các phần sau.
...
Ta có thể cài đặt KMP bằng cách sử dụng code trên: `prefix_function(s + # + t)` trả về hàm tiền tố cho $s + \text{\#} + t$ và ta chỉ cần đếm xem có bao nhiêu phần tử trong đó có giá trị là $n$. Cách cài đặt này tốn $\mathcal{O}(n + m)$ bộ nhớ.
...
```
Code trên lưu cả hai xâu $s$, $t$ nên vẫn dùng $\mathcal{O}(n + m)$ bộ nhớ, nhưng có thể giảm xuống còn $\mathcal{O}(n)$ bộ nhớ nếu ta đọc lần lượt từng ký tự của $t$ rồi tính luôn $\pi$ thay vì lưu cả xâu rồi mới tính.

Tổng kết lại, thuật toán KMP giải quyết được bài toán so khớp chuỗi trong thời gian $\mathcal{O}(n + m)$ và sử dụng $\mathcal{O}(n)$ bộ nhớ.
...
Do ta duyệt qua mỗi phần tử đúng một lần và độ phức tạp tính hàm tiền tố là $\mathcal{O}(n)$, tổng độ phức tạp thuật chỉ còn là $\mathcal{O}(n)$.
...
Vậy số xâu mới bị lặp với một xâu con có trước là $\pi_{\max}$, dẫn đến số xâu cần thêm vào đáp án là $i - \pi_{\max}$ (Do có $i$ xâu mới). Tính $\pi_{\max}$ với mỗi $i$ hết $\mathcal{O}(i)$ nên tổng độ phức tạp thuật toán là $\mathcal{O}(n^{2})$.

Ngoài ra, thay vì mỗi bước cập nhật kết quả sau khi thêm một chữ cái vào cuối xâu hiện tại, ta cũng có thể thêm một chữ vào đầu xâu hiện tại, hoặc bắt đầu với xâu hoàn chỉnh và mỗi bước bỏ đi chữ cái đầu (hoặc cuối). Độ phức tạp $\mathcal{O}(n^{2})$ cũng chưa phải là độ phức tạp tốt nhất - ta có thể đếm số xâu con của một xâu trong $\mathcal{O}(n)$ hoặc $\mathcal{O}(n \log n)$ nhờ mảng hậu tố (Suffix Array).
...
Nếu ta dùng thẳng điều kiện $(*)$), với mỗi giá trị của $l$ kiểm tra sẽ cần duyệt qua $\mathcal{O}(\frac{n}{l})$ cặp xâu liền kề, mỗi cặp kiểm tra xem chúng bằng nhau không mất độ phức tạp $\mathcal{O}(l)$, nên độ phức tạp để kiểm tra khi cố định $l$ là $\mathcal{O}(\frac{n}{l}) \times \mathcal{O}(l) = \mathcal{O}(n)$. $l$ chỉ có thể nhận $\mathcal{O}(n^{\frac{1}{3}})$ giá trị khác nhau do là ước của $n$, dẫn tới tổng độ phức tạp là $\mathcal{O}(n^{\frac{4}{3}})$. Độ phức tạp này là đủ tốt để AC với $n \leq 10^{5}$, nhưng ta có thể dùng hàm tiền tố để làm tốt hơn.
...
Hay nói cách khác, $l$ phải thỏa mãn tiền tố độ dài $n-l$ khớp với hậu tố có cùng độ dài của xâu. Kiểm tra điều kiện rút gọn này là đủ vì từ nó ta có thể suy ngược lại ra điều kiện $(*)$. Áp dụng hàm tiền tố: Các tiền tố mà cũng là hậu tố kết thúc tại $n-1$ có độ dài $\pi[n-1], \pi[\pi[n-1]-1], \pi[\pi[\pi[n-1]-1] - 1], \dots$, duyệt qua từng độ dài này và dừng lại khi có một độ dài $l$ thỏa mãn $n$ chia hết cho $n - l$, khi đó đáp án sẽ là $n - l$. Nếu không có độ dài nào thỏa mãn, đáp án khi đó là $n$. Độ phức tạp của thuật chỉ là $\mathcal{O}(n)$.
...
Xét xâu $s[k \dots (k+p-1)]$. Do tiền tố và hậu độ dài $\pi[n-1]$ bằng nhau, $s[k \dots (k+p-1)] = s[0 \dots p-1]$, so sánh từng cặp chữ cái tương ứng của hai xâu này, ta rút ra $s_i = s_{(i+k) \bmod p} ~ \forall ~ 0 \leq i \lt p$.

Đặt $d = \gcd(k,p)$, điều kiện trên khi đó tương đương với $s_i = s_{(i+d) \bmod p} ~ \forall ~ 0 \leq i \lt p$, nghĩa là $s[0 \dots d-1]$ là một xâu nén của $s[0 \dots p-1]$. Do $d \leq k \lt p$, $s[0 \dots d-1]$ là một xâu nén của $s$ có độ dài nhỏ hơn $p$, mâu thuẫn với cách chọn $p$ là xâu nén nhỏ nhất. Ta có điều phải chứng minh.
...
2. Do cấu trúc $g_i = g_{i-1} + \texttt{char}(i) + g_{i-1}$, khi duyệt đến hết nửa đầu của $g_i$ (duyệt qua các chữ cái của $g_{i-1}$) thì trạng thái đã chuyển từ $j$ sang $G[i-1][j]$.
3. Khi đọc đến $\texttt{char}(i)$, trạng thái mới dễ dàng tính được bằng ma trận chuyển trạng thái: $G[i-1][j] \rightarrow \texttt{mid}$, trong đó $\texttt{mid} = \texttt{aut}(G[i-1][j], \texttt{char}(i))$.
4. Cuối cùng, ta duyệt qua nửa bên phải của $g_i$, tương đương với duyệt qua $g_{i-1}$ một lần nữa, nên trạng thái cuối cùng là $G[i-1][\texttt{mid}]$.

Ta có thể tóm gọn cách trạng thái thay đổi bằng công thức:

$$
\texttt{mid} = \texttt{aut}[G[i-1][j]][i]
$$

$$
G[i][j] = G[i-1][\texttt{mid}]
$$

Biết được trạng thái thay đổi như thế nào, ta có thể dùng nó để tính lại đáp án khi đi từ $g_{i-1}$ đến $g_i$. Để thống nhất với cách ta tính $G$, đặt $K[i][j]$ là số lần xâu $s$ xuất hiện trong $g_i$, biết trạng thái ở đầu xâu là $j$. Khi duyệt đến giữa xâu, ta đang có $K[i][j] = K[i-1][j]$. Khi trạng thái chuyển thành $\texttt{mid}$, ta cộng $1$ vào $K[i][j]$ nếu $\texttt{mid} = \lvert s \rvert$. Phần còn lại chỉ là $K[i-1][\texttt{mid}]$. Công thức:

$$
K[i][j] = K[i-1][j] + (\texttt{mid} == |s|) + K[i-1][\texttt{mid}]
$$
