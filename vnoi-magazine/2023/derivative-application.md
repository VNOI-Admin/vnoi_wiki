...
$$
|10 - 10| \times 1000 + |20 - 10| \times 1 + |15 - 10| \times 2 = 20
$$
...
$$
f_i(x) = |x - H_i| \times C_i
$$

Đặt $F(x) = \sum_{i=1 \ldots n}{f_i(x)}$, nói cách khác $F(x)$ là chi phí để đưa tất cả các ngôi nhà về độ cao $x$. Đề bài yêu cầu ta tìm $\min(F(x))$.
...
Với nhận xét này, ta có thuật toán $\mathcal{O}(N \times \max_H)$, với $\max_H$ là giá trị lớn nhất của $H$: duyệt tất cả các giá trị của $x$, với mỗi $x$, ta tính hàm $F(x)$ trong $\mathcal{O}(N)$.
...
$\Rightarrow$ đến đây ta đã có thuật toán với độ phức tạp $\mathcal{O}(N\log(\max_H))$.

# Lời giải $\mathcal{O}(N + \max_H)$

Rất bất ngờ, bài này còn có thể giải được với độ phức tạp $\mathcal{O}(N + \max_H)$ sử dụng đạo hàm!
...
$f''_i(x) = 2 \times C(i)$ nếu $x = H(i+1)$, ngược lại $f''_i(x) = 0$.
...
Hàm $f''_i(x)$ chính là mấu chốt để giải bài toán này: hàm $f''_i(x)$ chỉ khác $0$ tại đúng 1 điểm, do đó ta dễ dàng "tính" được $f''_i(x)$ trong $\mathcal{O}(1)$.

$\Rightarrow$ ta có thể tính được $F''(x)$ trong $\mathcal{O}(N)$ rất đơn giản như sau:

```cpp
vector<long long> F2(MAX_H + 1); // F2[x] = F''(x)
for (int i = 0; i < N; ++i) {
...
Dựa vào định nghĩa của đạo hàm ($f'(x) = f(x) - f(x-1)$), từ $F''(x)$ ta có thể dễ dàng tính được $F'(x)$ trong $\mathcal{O}(\max_H)$:

```cpp
vector<long long> F1(MAX_H + 1); // F1[x] = F'(x)
// Tính F'(0)
for (int i = 0; i < N; ++i) {
    F1[0] -= C[i];
}
...
Tương tự, ta có thể tính được $F(x)$ trong $\mathcal{O}(\max_H)$:

```cpp
vector<long long> F(MAX_H + 1);
// Tính F(0)
for (int i = 0; i < N; ++i) {
    F[0] += H[i] * C[i];
}
...
Đến đây ta đã thu được thuật toán với độ phức tạp $\mathcal{O}(N + \max_H)$!
